...
def test_imports() -> Dict[str, Any]:
    ...
    try:
        ...
        print("    OK: qt_material_ui imported successfully")
        ...
        for theme, variant in themes_to_test:
            if theme in available_themes and variant in available_themes[theme]:
                ...
                if success:
                    ...
                    print(f"    OK: Applied theme: {theme}/{variant}")
                else:
...
def test_performance() -> Dict[str, Any]:
    ...
    try:
        ...
        for theme, variant in themes_to_test:
            ...
            if success:
                if switch_time < 100:
                    print(f"    OK: {theme}/{variant}: {switch_time:.2f}ms (<100ms target)")
                else:
                    print(f"    WARN: {theme}/{variant}: {switch_time:.2f}ms (exceeds 100ms target)")
                    results["success"] = False
            else:
                print(f"    FAIL: Failed to apply theme: {theme}/{variant}")
                results["success"] = False
...
def test_circular_dependency_resolution() -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"    OK: Service-first import: {theme1}/{variant1}")
        ...
        print(f"    OK: Core-first import: {theme2}/{variant2}")
        ...
        results["errors"].append(traceback.format_exc())
        print(f"    FAIL: Circular dependency test failed: {e}")
...
def main():
    ...
    if passed_tests == total_tests:
        print("SUCCESS: All tests passed! Qt-material takeover is successful.")
        print("\nKey achievements:")
