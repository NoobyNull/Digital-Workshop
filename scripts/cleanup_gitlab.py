...
def get_gitlab_token():
    ...
    print(
        "Please set GITLAB_TOKEN environment variable with your GitLab personal access token"
    )
    print(
        "You can create one at: https://gitlab.yax.family/-/profile/personal_access_tokens"
    )
    print()
...
def get_failed_jobs(gitlab_url, token):
    ...
    try:
        ...
        response = requests.get(
            f"{gitlab_url}/api/v4/jobs?scope=failed", headers=headers, timeout=10
        )
    ...
    try:
        ...
        for job_id in job_ids:
            response = requests.post(
                f"{gitlab_url}/api/v4/jobs/{job_id}/cancel", headers=headers, timeout=10
            )
...
def retry_pipeline(gitlab_url, token):
    ...
    try:
        ...
        response = requests.post(
            f"{gitlab_url}/api/v4/projects/mty%2Fdigital-workshop/trigger/pipeline",
            headers=headers,
            timeout=10,
        )
...
def cleanup_artifacts(gitlab_url, token, keep_recent=5):
    ...
    try:
        ...
        response = requests.get(
            f"{gitlab_url}/api/v4/projects/mty%2Fdigital-workshop/jobs?per_page=50",
            headers=headers,
            timeout=10,
        )
        ...
        if response.status_code == 200:
            ...
            for job in jobs:
                if (
                    ...
                    and len(jobs_to_cleanup) < keep_recent
                ):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Clean up GitLab failed builds and restart pipeline"
    )
    parser.add_argument(
        "--gitlab-url", help="GitLab URL (default: https://gitlab.yax.family)"
    )
    parser.add_argument("--token", help="GitLab personal access token")
    parser.add_argument(
        "--keep-recent", type=int, default=5, help="Number of recent jobs to keep"
    )
    parser.add_argument(
        "--cleanup-only",
        action="store_true",
        help="Only cleanup, do not retry pipeline",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    print("\n✅ Cleanup completed!")
    print(
        "📊 Check GitLab CI/CD at: https://gitlab.yax.family/mty/digital-workshop/-/pipelines"
    )
