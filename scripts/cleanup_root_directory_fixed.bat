@echo off
REM Digital Workshop Root Directory Cleanup Script (Windows Version - Fixed)
REM Execute this script to perform the complete cleanup
REM 
REM Usage: cleanup_root_directory_fixed.bat
REM 
...
for /f %%i in ('dir /b /a-d 2^>nul ^| find /c /v ""') do set BEFORE_COUNT=%%i
echo Current root directory file count: %BEFORE_COUNT% files

REM Phase 1: Backup
echo.
echo Phase 1: Creating backup...
set BACKUP_DIR=..\digital-workshop-backup-20251031-213805
...
if exist "QUICK_START_GUIDE.md" (
    move "QUICK_START_GUIDE.md" "docs\guides\QUICK_START.md" >nul 2>&1
    if !errorlevel! equ 0 (
...
if exist "REFACTORING_SOLUTIONS.md" (
    move "REFACTORING_SOLUTIONS.md" "docs\guides\REFACTORING.md" >nul 2>&1
    if !errorlevel! equ 0 (
...
if exist "IMPORT_PROCESS_ARCHITECTURE.md" (
    move "IMPORT_PROCESS_ARCHITECTURE.md" "docs\architecture\IMPORT_PROCESS.md" >nul 2>&1
    if !errorlevel! equ 0 (
...
if exist "FINAL_IMPLEMENTATION_REPORT.md" (
    move "FINAL_IMPLEMENTATION_REPORT.md" "docs\reports\FINAL_IMPLEMENTATION.md" >nul 2>&1
    if !errorlevel! equ 0 (
...
if exist "WINDOW_PERSISTENCE_TEST_GUIDE.md" (
    move "WINDOW_PERSISTENCE_TEST_GUIDE.md" "docs\guides\WINDOW_PERSISTENCE.md" >nul 2>&1
    if !errorlevel! equ 0 (
...
if exist "vtk_resource_tracker_fix_summary.md" (
    move "vtk_resource_tracker_fix_summary.md" "docs\reports\VTK_RESOURCE_TRACKER.md" >nul 2>&1
    if !errorlevel! equ 0 (
...
if exist "comprehensive_test_suite.py" (
    move "comprehensive_test_suite.py" "tests\integration\" >nul 2>&1
    if !errorlevel! equ 0 (
...
if exist "comprehensive_test_suite_tests.py" (
    move "comprehensive_test_suite_tests.py" "tests\integration\" >nul 2>&1
    if !errorlevel! equ 0 (
...
if exist "unified_test_runner.py" (
    move "unified_test_runner.py" "tools\maintenance\" >nul 2>&1
    if !errorlevel! equ 0 (
...
if exist "quality_config.yaml" (
    move "quality_config.yaml" "config\" >nul 2>&1
    if !errorlevel! equ 0 (
...
if exist "test_framework_config.json" (
    move "test_framework_config.json" "config\" >nul 2>&1
    if !errorlevel! equ 0 (
...
if exist "pyinstaller.spec" (
    move "pyinstaller.spec" "build_system\" >nul 2>&1
    if !errorlevel! equ 0 (
...
if exist "installer.nsi" (
    move "installer.nsi" "config\" >nul 2>&1
    if !errorlevel! equ 0 (
...
if exist "sample_large_module.py" (
    move "sample_large_module.py" "samples\code\" >nul 2>&1
    if !errorlevel! equ 0 (
...
if exist "sample_small_module.py" (
    move "sample_small_module.py" "samples\code\" >nul 2>&1
    if !errorlevel! equ 0 (
...
if exist "installer" (
    move "installer" "build\installer\" >nul 2>&1
    if !errorlevel! equ 0 (
...
for %%f in (*_analysis.json) do (
    if exist "%%f" (
        move "%%f" "reports\analysis\" >nul 2>&1
        if !errorlevel! equ 0 (
            echo Moved: %%f
            set /a MOVED_COUNT+=1
        )
...
for %%f in (comprehensive_*_report.md) do (
    if exist "%%f" (
        move "%%f" "reports\comprehensive\" >nul 2>&1
        if !errorlevel! equ 0 (
            echo Moved: %%f
            set /a MOVED_COUNT+=1
        )
...
for %%f in (performance_*.json) do (
    if exist "%%f" (
        move "%%f" "reports\performance\" >nul 2>&1
        if !errorlevel! equ 0 (
            echo Moved: %%f
            set /a MOVED_COUNT+=1
        )
...
for %%f in (naming_*.json) do (
    if exist "%%f" (
        move "%%f" "reports\quality\" >nul 2>&1
        if !errorlevel! equ 0 (
            echo Moved: %%f
            set /a MOVED_COUNT+=1
        )
...
for %%f in (test_results.*) do (
    if exist "%%f" (
        move "%%f" "reports\test_results\" >nul 2>&1
        if !errorlevel! equ 0 (
            echo Moved: %%f
            set /a MOVED_COUNT+=1
        )
...
if exist "code_quality_validator.py" (
    move "code_quality_validator.py" "tools\maintenance\" >nul 2>&1
    if !errorlevel! equ 0 (
...
if exist "naming_validator.py" (
    move "naming_validator.py" "tools\maintenance\" >nul 2>&1
    if !errorlevel! equ 0 (
...
if exist "quality_gate_enforcer.py" (
    move "quality_gate_enforcer.py" "tools\maintenance\" >nul 2>&1
    if !errorlevel! equ 0 (
...
if exist "monolithic_detector.py" (
    move "monolithic_detector.py" "tools\maintenance\" >nul 2>&1
    if !errorlevel! equ 0 (
...
if exist "debug_detection.py" (
    move "debug_detection.py" "tools\debug\" >nul 2>&1
    if !errorlevel! equ 0 (
...
if exist "create_exceptions.py" (
    move "create_exceptions.py" "tools\exceptions\" >nul 2>&1
    if !errorlevel! equ 0 (
...
if exist "migrate_models.py" (
    move "migrate_models.py" "tools\migration\" >nul 2>&1
    if !errorlevel! equ 0 (
...
if exist "monolithic_report.json" (
    move "monolithic_report.json" "tools\analysis\" >nul 2>&1
    if !errorlevel! equ 0 (
...
for /f %%i in ('dir /b /a-d 2^>nul ^| find /c /v ""') do set AFTER_COUNT=%%i
...
set SUMMARY_FILE=cleanup_summary_20251031_213805.txt

echo Digital Workshop Root Directory Cleanup Summary > "%SUMMARY_FILE%"
echo =============================================== >> "%SUMMARY_FILE%"
echo Date: 2025-10-31 21:38:05 >> "%SUMMARY_FILE%"
