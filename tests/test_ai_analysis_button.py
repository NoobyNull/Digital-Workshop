...
class TestAIAnalysisButton:
    ...
    def test_run_ai_analysis_button_exists(self):
        ...
        try:
            from src.gui.metadata_components.metadata_editor_main import (
                MetadataEditorWidget,
            )

            with patch(
                "src.gui.metadata_components.metadata_editor_main.get_database_manager"
            ):
                editor = MetadataEditorWidget()

                # Check button exists
                assert hasattr(
                    editor, "run_ai_analysis_button"
                ), "Run AI Analysis button not found"
                assert (
                    editor.run_ai_analysis_button is not None
                ), "Run AI Analysis button is None"
    ...
    def test_run_ai_analysis_button_connected(self):
        ...
        try:
            ...
            with patch(
                ...
                ), "_run_ai_analysis method not found"
                assert callable(
                    editor._run_ai_analysis
                ), "_run_ai_analysis is not callable"
    ...
    def test_apply_ai_results_updates_fields(self):
        ...
        try:
            ...
            with patch(
                ...
                assert (
                    editor.description_edit.toPlainText() == "Test Description"
                ), "Description not updated"
                assert (
                    editor.keywords_edit.text() == "keyword1, keyword2, keyword3"
    ...
    def test_ai_analysis_button_tooltip(self):
        ...
        try:
            ...
            with patch(
                ...
                assert (
                    "AI" in tooltip or "analysis" in tooltip.lower()
                ), "Tooltip doesn't mention AI analysis"
