...
def test_preferences_default_material():
    ...
    try:
        ...
        if loaded_material == test_material:
            ...
            logger.error(
                f"✗ Preferences default material test FAILED: expected '{test_material}', got '{loaded_material}'"
            )
            return False
...
def test_model_loader_integration():
    ...
    try:
        ...
        mock_main_window.material_manager.get_species_list.return_value = [
            "maple",
            "cherry",
            "oak",
        ]
        ...
        if hasattr(model_loader, "_apply_default_material_from_preferences"):
            logger.info(
                "✓ ModelLoader has _apply_default_material_from_preferences method"
            )
        else:
            logger.error(
                "✗ ModelLoader missing _apply_default_material_from_preferences method"
            )
            return False
        ...
        try:
            ...
            logger.info(
                "✓ _apply_default_material_from_preferences method executes without errors"
            )
        except Exception as e:
            logger.error(
                f"✗ _apply_default_material_from_preferences method failed: {e}"
            )
            return False
...
def test_material_system_functionality():
    ...
    try:
        ...
        methods_to_test = ["apply_stl_material_properties", "parse_mtl_direct"]
        ...
        for method_name in methods_to_test:
            if hasattr(integrator, method_name):
                ...
                logger.error(
                    f"✗ MaterialLightingIntegrator missing method: {method_name}"
                )
