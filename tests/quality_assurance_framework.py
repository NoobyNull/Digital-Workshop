...
class QualityAssuranceEngine:
    ...
    def run_comprehensive_quality_assessment(self) -> CodeQualityReport:
        ...
        test_coverage_score = self._calculate_component_score(
            [m for m in metrics if "coverage" in m.name.lower()], "test_coverage"
        )

        code_quality_score = self._calculate_component_score(
            [m for m in metrics if m.name.startswith("code_")], "code_quality"
        )

        performance_score = self._calculate_component_score(
            [m for m in metrics if m.name.startswith("perf_")], "performance"
        )
        ...
        self.logger.info(
            f"Quality assessment completed. Overall score: {overall_score:.1f}/100"
        )
    ...
    def _analyze_test_coverage(self) -> List[QualityMetric]:
        ...
        try:
            ...
            metrics.append(
                QualityMetric(
                    ...
                    status=(
                        "PASS"
                        if overall_coverage
                        >= self.quality_thresholds["test_coverage_min"]
                        else "FAIL"
                    ),
            ...
            for module in critical_modules:
                module_coverage = coverage_data.get("module_coverage", {}).get(
                    module, 0.0
                )
                status = (
                    "PASS"
                    if module_coverage >= self.quality_thresholds["test_coverage_min"]
                    else "FAIL"
                )
    ...
    def _run_coverage_analysis(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.coverage_file.exists():
                ...
                total_lines = sum(
                    f.get("summary", {}).get("num_statements", 0)
                    for f in coverage_data.get("files", {}).values()
                )
                covered_lines = sum(
                    f.get("summary", {}).get("covered_lines", 0)
                    for f in coverage_data.get("files", {}).values()
                )
                ...
                for module in module_coverage:
                    module_coverage[module] = sum(module_coverage[module]) / len(
                        module_coverage[module]
                    )
    ...
    def _analyze_code_quality(self) -> List[QualityMetric]:
        ...
        source_files = [
            f for f in source_files if not f.is_relative_to(self.project_root / "tests")
        ]
        ...
        try:
            ...
            for file_path in source_files:
                try:
                    ...
                    self.logger.warning(
                        f"Failed to analyze complexity for {file_path}: {e}"
                    )
                ...
                metrics.append(
                    QualityMetric(
                        ...
                        status=(
                            "PASS"
                            if avg_complexity
                            <= self.quality_thresholds["code_complexity_max"]
                            else "FAIL"
                        ),
    ...
    def _analyze_code_duplication(
        ...
        try:
            ...
            duplicates = {
                content: files
                for content, files in content_counts.items()
                if len(files) > 1
            }

            duplication_percentage = (
                (len(duplicates) / len(file_contents)) * 100 if file_contents else 0
            )
            ...
            metrics.append(
                QualityMetric(
                    ...
                    status=(
                        ...
                        <= self.quality_thresholds["code_duplication_max"]
                        else "FAIL"
                    ),
    ...
    def _analyze_code_style(self, source_files: List[Path]) -> List[QualityMetric]:
        ...
        try:
            ...
            critical_issues = [
                issue for issue in style_issues if issue.get("severity") == "high"
            ]
    ...
    def _analyze_performance_quality(self) -> List[QualityMetric]:
        ...
        performance_reports = list(
            self.project_root.glob("reports/performance/benchmark_results_*.json")
        )
        ...
        if performance_reports:
            ...
            try:
                ...
                regression_rate = (
                    (regressions / total_tests * 100) if total_tests > 0 else 0
                )
                ...
                metrics.append(
                    QualityMetric(
                        ...
                        status=(
                            "PASS"
                            if regression_rate
                            <= self.quality_thresholds["performance_regression_max"]
                            else "FAIL"
                        ),
    ...
    def _analyze_security_quality(self) -> List[QualityMetric]:
        ...
        try:
            ...
            source_files = [
                ...
                if not f.is_relative_to(self.project_root / "tests")
            ]
            ...
            for file_path in source_files:
                try:
                    ...
                    self.logger.warning(
                        f"Failed to analyze security in {file_path}: {e}"
                    )
            ...
            metrics.append(
                QualityMetric(
                    ...
                    status=(
                        ...
                        <= self.quality_thresholds["security_vulnerabilities_max"]
                        else "FAIL"
                    ),
    ...
    def _calculate_component_score(
        self, metrics: List[QualityMetric], component_name: str
    ) -> float:
    ...
    def _calculate_reliability_score(self, metrics: List[QualityMetric]) -> float:
        ...
        reliability_metrics = [
            ...
            if any(
                ...
                for keyword in ["test_", "coverage", "failure"]
            )
        ]
    ...
    def _calculate_maintainability_score(self, metrics: List[QualityMetric]) -> float:
        ...
        maintainability_metrics = [
            ...
            if any(
                keyword in m.name.lower()
                for keyword in ["complexity", "duplication", "style"]
            )
        ]
    ...
    def _generate_quality_summary(
        ...
        if any(
            m.name.startswith("test_coverage") and m.status == "FAIL" for m in metrics
        ):
    ...
    def check_quality_gates(self, report: CodeQualityReport) -> Tuple[bool, List[str]]:
        ...
        if report.overall_score < 70.0:
            failures.append(
                f"Overall quality score {report.overall_score:.1f} below minimum threshold 70.0"
            )

        # Test coverage gate
        if report.test_coverage < self.quality_thresholds["test_coverage_min"]:
            failures.append(
                f"Test coverage {report.test_coverage:.1f}% below minimum threshold {self.quality_thresholds['test_coverage_min']}%"
            )

        # Security gate
        if report.security_score < 80.0:
            failures.append(
                f"Security score {report.security_score:.1f} below minimum threshold 80.0"
            )

        # Performance gate
        if report.performance_score < 60.0:
            failures.append(
                f"Performance score {report.performance_score:.1f} below minimum threshold 60.0"
            )

        # Critical failures
        critical_failures = [
            m for m in report.metrics if m.status == "FAIL" and m.weight >= 2.0
        ]
        if critical_failures:
            failures.append(
                f"Critical quality failures detected: {len(critical_failures)}"
            )
...
class QualityAssuranceReporter:
    ...
    def generate_markdown_report(self, report: CodeQualityReport) -> str:
        ...
        lines.append(
            f"**Overall Score:** {report.overall_score:.1f}/100 ({report.summary['quality_grade']})"
        )
        lines.append("")
        ...
        lines.append(
            f"- **Passed:** {summary['passed_metrics']} ({summary['success_rate']:.1f}%)"
        )
        ...
        for metric in report.metrics:
            status_icon = (
                "✅"
                if metric.status == "PASS"
                else "⚠️" if metric.status == "WARN" else "❌"
            )
            lines.append(
                f"| {metric.name} | {metric.value:.1f} | {metric.threshold:.1f} | {status_icon} {metric.status} | {metric.weight:.1f} |"
            )
