...
class ProgressTracker:
    ...
    def _display_progress(self, progress: ProgressInfo):
        ...
        if progress.current_tool:
            print(
                f"\r[{progress.completed_tools}/{progress.total_tools}] "
                f"Running {progress.current_tool}: {progress.current_step} "
                f"({progress.progress_percentage:.1f}%) "
                f"ETA: {progress.estimated_time_remaining:.0f}s",
                end="",
                flush=True,
            )
        else:
...
class ComprehensiveTestSuite:
    ...
    def __init__(self, config_path: Optional[Path] = None):
        ...
        self.tools = {
            "monolithic_detector": {
                ...
                .get("monolithic_detector", {})
                .get("enabled", True),
                "args": self._get_monolithic_args(),
                "timeout": self.config.get("tools", {})
                .get("monolithic_detector", {})
                .get("timeout", 300),
            },
            "naming_validator": {
    ...
    def _get_monolithic_args(self) -> List[str]:
        ...
        args = [
            ...
            "--threshold",
            str(tool_config.get("threshold", 500)),
            "--output",
            f"{self.config['output_dir']}/monolithic_report.json",
            "--workers",
            str(tool_config.get("workers", 4)),
        ]
        return args
    ...
    def _get_naming_args(self) -> List[str]:
        ...
        args = [
            self.config["target_path"],
            "--output",
            f"{self.config['output_dir']}/naming_report.json",
            "--workers",
            str(tool_config.get("workers", 4)),
            "--min-compliance",
            str(tool_config.get("min_compliance", 95.0)),
        ]
        return args
    ...
    def _get_test_runner_args(self) -> List[str]:
        ...
        args = [
            "--output",
            f"{self.config['output_dir']}/test_report.json",
            "--max-workers",
            str(tool_config.get("max_workers", 4)),
        ]
    ...
    def _get_quality_args(self) -> List[str]:
        ...
        args = [
            self.config["target_path"],
            "--output",
            f"{self.config['output_dir']}/quality_report.json",
        ]
    ...
    def _get_gate_args(self) -> List[str]:
        ...
        args = ["--output", f"{self.config['output_dir']}/quality_gate_report.json"]
    ...
    def execute_tool(self, tool_name: str, tool_config: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            metrics, violations, recommendations = self._parse_tool_output(
            ...
            self.progress_tracker.update_progress(
                tool_name, "Completed", completed=True
            )
    ...
    def run_all_tools(
        ...
        enabled_tools = {
            name: config for name, config in self.tools.items() if config["enabled"]
        }
        ...
        if parallel:
            ...
            with ThreadPoolExecutor(
                max_workers=max_workers or len(enabled_tools)
            ) as executor:
                future_to_tool = {
                    executor.submit(
                        self.execute_tool, tool_name, tool_config
                    ): tool_name
        ...
        for result in results:
            if result.tool_name == "monolithic_detector":
                ...
                compliance_scores.append(
                    100.0
                    if result.metrics.get("monolithic_files_found", 0) == 0
                    else 0.0
                )
                ...
                if gate_results:
                    passed_gates = sum(
                        1 for gate in gate_results if gate.get("passed", False)
                    )
    ...
    def _generate_unified_recommendations(self, results: List[ToolResult]) -> List[str]:
        ...
        if failed_tools:
            recommendations.append(
                f"Address failures in {len(failed_tools)} tool(s): {', '.join(r.tool_name for r in failed_tools)}"
            )
        ...
        if total_time > 600:  # 10 minutes
            recommendations.append(
                "Consider running tools in parallel to improve performance"
            )
        ...
        for result in results:
            if result.tool_name == "naming_validator":
                ...
                if compliance < 95:
                    recommendations.append(
                        f"Improve naming convention compliance (currently {compliance:.1f}%)"
                    )
            elif result.tool_name == "monolithic_detector":
                violations = result.metrics.get("monolithic_files_found", 0)
                if violations > 0:
                    recommendations.append(
                        f"Refactor {violations} monolithic modules to improve code organization"
                    )
    ...
    def _generate_next_steps(self, results: List[ToolResult]) -> List[str]:
        ...
        for result in results:
            if not result.success:
                ...
                result.tool_name == "monolithic_detector"
                and result.metrics.get("monolithic_files_found", 0) > 0
            ):
                critical_issues += 1
...
"""
    ...
    def _print_console_summary(self, report: Dict[str, Any]):
        ...
        for result in report["tool_results"]:
            ...
            print(
                f"{result['tool_name'].replace('_', ' ').title():<30} {status} ({result['execution_time']:.2f}s)"
            )
            if result["error_message"]:
    ...
    def _configure_tools_interactive(self):
        ...
        for tool_name, tool_config in self.tools.items():
            ...
            new_enabled = (
                input(f"  Change enabled status? (current: {current_enabled}): ")
                .strip()
                .lower()
            )
            ...
            if tool_name == "monolithic_detector":
                threshold = input(
                    f"  Threshold (current: {tool_config.get('threshold', 500)}): "
                ).strip()
                if threshold.isdigit():
                    tool_config["threshold"] = int(threshold)
            elif tool_name == "naming_validator":
                min_compliance = input(
                    f"  Min compliance % (current: {tool_config.get('min_compliance', 95.0)}): "
                ).strip()
...
Examples:
  ...
  python comprehensive_test_suite.py --sequential --target src/
    ...
    parser.add_argument(
        "--parallel", "-p", action="store_true", help="Run tools in parallel (default)"
    )

    parser.add_argument(
        "--sequential", "-s", action="store_true", help="Run tools sequentially"
    )
    ...
    parser.add_argument(
        "--target", "-t", type=str, default=".", help="Target directory to analyze"
    )
