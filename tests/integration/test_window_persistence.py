...
def test_qsettings_consistency():
    ...
    ), f"Org mismatch: {org} != {config.organization_name}"
...
def test_window_size_persistence():
    ...
    print(
        f"Saved: width={test_width}, height={test_height}, maximized={test_maximized}"
    )
    ...
    print(
        f"Loaded: width={loaded_width}, height={loaded_height}, maximized={loaded_maximized}"
    )

    assert loaded_width == test_width, f"Width mismatch: {loaded_width} != {test_width}"
    assert (
        loaded_height == test_height
    ), f"Height mismatch: {loaded_height} != {test_height}"
    assert (
        loaded_maximized == test_maximized
    ), f"Maximized mismatch: {loaded_maximized} != {test_maximized}"
