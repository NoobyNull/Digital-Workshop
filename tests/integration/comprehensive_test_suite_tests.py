...
"""
    ...
    def test_tool_execution_success(self, mock_popen, temp_dir, mock_tools):
        ...
        result = suite.execute_tool(
            "monolithic_detector", suite.tools["monolithic_detector"]
        )
    ...
    def test_interactive_mode_menu(self, temp_dir):
        ...
        config_path.write_text(
            '{"target_path": ".", "output_dir": "reports", "tools": {}}'
        )
    ...
    def test_configuration_management(self, temp_dir):
        ...
        initial_config = {
            "target_path": temp_dir,
            "output_dir": f"{temp_dir}/reports",
            "tools": {"monolithic_detector": {"enabled": True, "threshold": 100}},
        }
    ...
    def test_error_handling_and_recovery(self, temp_dir):
        ...
        result = suite.execute_tool(
            "nonexistent_tool", {"script": "nonexistent.py", "args": [], "timeout": 10}
        )
    ...
    def test_ci_cd_integration_features(self, temp_dir):
        ...
        config = {
            "target_path": temp_dir,
            "output_dir": f"{temp_dir}/reports",
            "tools": {"monolithic_detector": {"enabled": True, "timeout": 10}},
            "ci_cd": {
                "enabled": True,
            ...
            json.dumps(
                {
                    "target_path": temp_dir,
                    "output_dir": f"{temp_dir}/reports",
                    "tools": {"monolithic_detector": {"enabled": True, "timeout": 5}},
                    "performance": {"max_total_time": 10, "memory_limit_mb": 512},
                }
            )
        )
...
def run_integration_tests():
    ...
    pytest_args = [
        ...
        "--self-contained-html",
        "--junit-xml",
        f"{results_dir}/junit_results.xml",
    ]
