...
class TestImportAnalysisService(unittest.TestCase):
    ...
    def test_count_unique_vertices(self):
        ...
        tri1 = Triangle(
            Vector3D(0, 0, 1), Vector3D(0, 0, 0), Vector3D(1, 0, 0), Vector3D(0, 1, 0)
        )
        tri2 = Triangle(
            Vector3D(0, 0, 1),
    ...
    def test_surface_area_calculation(self):
        ...
        tri2 = Triangle(
            Vector3D(0, 0, 1), Vector3D(0, 0, 0), Vector3D(1, 0, 0), Vector3D(0, 0, 1)
        )
    ...
    def test_count_degenerate_triangles(self):
        ...
        degenerate_count = self.service._count_degenerate_triangles(
            [degenerate_tri, normal_tri]
        )
...
class TestBatchAnalysis(unittest.TestCase):
    ...
    def test_batch_analysis_statistics(self):
        ...
        with patch.object(
            self.service, "_analyze_single_model", side_effect=mock_results
        ):
            result = self.service._analyze_batch_internal(
                [("file1.stl", 1), ("file2.stl", 2), ("file3.stl", 3)], None, None
            )
