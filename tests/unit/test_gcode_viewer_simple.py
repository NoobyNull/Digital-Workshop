...
def test_imports():
    ...
    try:
        # Test parser import
        from src.gui.gcode_previewer_components.gcode_parser import (
            GcodeParser,
            GcodeMove,
        )
        ...
        from src.gui.gcode_previewer_components.camera_controller import (
            CameraController,
        )
...
def test_renderer():
    ...
    try:
        ...
        if hasattr(renderer, "move_data"):
            print(
                f"✓ Move data structure exists with {len(renderer.move_data)} categories"
            )
