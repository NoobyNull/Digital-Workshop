...
class TestCodeQualityValidator(unittest.TestCase):
    ...
    def test_format_with_black_changes_needed(self, mock_run):
        ...
        )  # No changes recorded when fix_mode=False
    ...
    def test_lint_with_pylint_no_issues(self, mock_run):
        ...
        mock_run.return_value = Mock(
            stdout="[]", stderr="Your code has been rated at 10.00/10"
        )
    ...
    def test_calculate_compliance_perfect_score(self):
        ...
        self.assertAlmostEqual(
            compliance.linting_score, 97.5, places=2
        )  # (10.0 + 9.5) / 2 * 10
        self.assertAlmostEqual(
            compliance.overall_score, 98.25, places=2
        )  # 100 * 0.3 + 97.5 * 0.7
    ...
    def test_calculate_compliance_below_threshold(self):
        ...
        formatting_results = [
            FormattingResult(
                "file1.py", True, 100, 95, ["formatting"], 0.5
            ),  # Needed formatting
        ]
        ...
        self.assertAlmostEqual(
            compliance.formatting_score, 85.0, places=2
        )  # Penalized for needing formatting
        self.assertAlmostEqual(compliance.linting_score, 50.0, places=2)  # 5.0 * 10
        self.assertAlmostEqual(
            compliance.overall_score, 60.5, places=2
        )  # 85 * 0.3 + 50 * 0.7 = 25.5 + 35 = 60.5
    ...
    def test_validate_file_integration(self):
        ...
        with patch.object(
            self.validator, "format_with_black"
        ) as mock_format, patch.object(self.validator, "lint_with_pylint") as mock_lint:
            ...
            formatting_result, linting_result = self.validator.validate_file(
                self.test_file
            )
...
class TestPerformanceRequirements(unittest.TestCase):
    ...
    def test_large_file_processing_time(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = Mock(
                returncode=0, stdout="", stderr="Your code has been rated at 8.00/10"
            )
    ...
    def test_memory_efficiency(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = Mock(
                returncode=0, stdout="[]", stderr="Your code has been rated at 9.00/10"
            )
