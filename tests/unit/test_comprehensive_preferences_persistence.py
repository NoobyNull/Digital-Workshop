...
class TestController(QMainWindow):
    ...
    def _show_welcome(self):
        ...
        self._log(
            "  2. Content tab settings persistence (background + material)", "INFO"
        )
        ...
        self._log("")
        self._log(
            "Click 'Start Comprehensive Test Suite' to begin automated testing.", "INFO"
        )
    ...
    def _run_test_1(self):
        ...
        try:
            ...
            self._log(
                "Step 2: Opening preferences (should default to General tab)...", "INFO"
            )
            ...
            if current_tab == 3:
                ...
                self._log(
                    f"✗ Failed to navigate to Content tab. Current: {current_tab}",
                    "FAIL",
                )
            ...
            if content_tab.bg_list.count() > 0:
                ...
                self._log(
                    f"Selected background: {Path(bg_path).stem if bg_path else 'None'}",
                    "INFO",
                )
            ...
            self._log(
                f"  Saved background: {Path(saved_bg).stem if saved_bg else 'None'}",
                "INFO",
            )
            ...
            restored_bg = (
                restored_bg_item.data(Qt.UserRole) if restored_bg_item else None
            )
            restored_mat = content_tab2.material_combo.currentData()

            self._log(f"  Restored tab index: {restored_tab}", "INFO")
            self._log(
                f"  Restored background: {Path(restored_bg).stem if restored_bg else 'None'}",
                "INFO",
            )
            ...
            if restored_tab == 3:
                ...
                self._log(
                    f"✗ Tab persistence FAILED (expected 3, got {restored_tab})", "FAIL"
                )
                test_passed = False
    ...
    def _run_test_3(self):
        ...
        try:
            ...
            if test_passed:
                ...
                self._log(
                    f"✗ Cross-tab interaction test FAILED (expected 3, got {final_tab})",
                    "FAIL",
                )
    ...
    def _show_test_summary(self):
        ...
        if passed == total:
            self._log(
                "🎉 ALL TESTS PASSED! Preferences persistence is fully functional!",
                "PASS",
            )
        else:
            self._log(
                f"⚠ {total - passed} test(s) failed. Review logs above for details.",
                "WARN",
            )
