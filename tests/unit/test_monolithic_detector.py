...
class TestMonolithicDetector(unittest.TestCase):
    ...
    def test_basic_monolithic_detection(self):
        ...
        monolithic_file = self.create_test_file(
            "monolithic_module.py", monolithic_content
        )
        ...
        # Verify results
        self.assertFalse(
            small_metrics.is_monolithic, "Small module should not be flagged"
        )
        self.assertFalse(
            medium_metrics.is_monolithic, "Medium module should not be flagged"
        )
        self.assertTrue(large_metrics.is_monolithic, "Large module should be flagged")
        self.assertTrue(
            monolithic_metrics.is_monolithic, "Monolithic module should be flagged"
        )

        # Verify severity levels
        self.assertEqual(
            large_metrics.severity, "minor", "Large module should have minor severity"
        )
        self.assertEqual(
            monolithic_metrics.severity,
            "critical",
            "Monolithic module should have critical severity",
        )
...
"""
        ...
        self.assertLessEqual(
            metrics.code_lines, 2, "Syntax error file should return minimal code lines"
        )
        self.assertFalse(
            metrics.is_monolithic, "Syntax error file should not be flagged"
        )
    ...
    def test_directory_scanning(self):
        ...
        self.assertEqual(
            len(results), 5, "Should analyze all Python files in directory"
        )
        self.assertTrue(
            all(isinstance(r, ModuleMetrics) for r in results),
    ...
    def test_performance_large_codebase(self):
        ...
        self.assertLess(
            processing_time, 30, "Should process 100 files in under 30 seconds"
        )
    ...
    def test_custom_threshold(self):
        ...
        self.assertFalse(
            metrics_200.is_monolithic, "Should not be flagged with threshold 200"
        )
    ...
    def test_report_generation(self):
        ...
        self.assertIn(
            "detailed_results", report, "Report should contain detailed results"
        )
    ...
    def test_parallel_processing(self):
        ...
        for workers in [1, 2, 4]:
            ...
            self.assertEqual(
                len(results), 10, f"Should analyze all files with {workers} workers"
            )
...
class TestIntegrationScenarios(unittest.TestCase):
    ...
    def test_realistic_codebase_analysis(self):
        ...
        self.assertFalse(
            small_result.is_monolithic, "Small module should not be flagged"
        )
        self.assertTrue(large_result.is_monolithic, "Large module should be flagged")
        self.assertTrue(
            monolithic_result.is_monolithic, "Monolithic module should be flagged"
        )
