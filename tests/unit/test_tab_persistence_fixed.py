...
class TabPersistenceTester:
    ...
    def test_basic_persistence(self) -> None:
        ...
        self.assert_equal(initial_tab, 0, "Dialog opens at General tab (index 0) initially")
        ...
        dialog2.close()
        self.log("✓ Basic persistence test complete")
    ...
    def test_multiple_tab_switching(self) -> None:
        ...
        dialog2.close()
        self.log("✓ Multiple tab switching test complete")
    ...
    def test_invalid_index_handling(self) -> None:
        ...
        self.assert_equal(current, last_valid_index, f"Valid edge case index ({last_valid_index}) works correctly")
        dialog3.close()
        
        self.log("✓ Invalid index handling test complete")
    ...
    def test_settings_storage_verification(self) -> None:
        ...
        self.assert_equal(has_key_after, True, "QSettings key 'preferences/last_tab_index' exists")
        
        self.log("✓ Settings storage verification test complete")
    ...
    def test_application_restart_simulation(self) -> None:
        ...
        self.assert_equal(current3, 1, "Session 3: Appearance tab restored from previous session")
        dialog3.close()
        
        self.log("✓ Application restart simulation test complete")
    ...
    def print_summary(self) -> None:
        ...
        self.log(f"Passed: {self.passed_count} [PASS]", "PASS")
        self.log(f"Failed: {self.failed_count} [FAIL]", "FAIL" if self.failed_count > 0 else "INFO")
        ...
        if self.failed_count > 0:
            ...
            for result in self.results:
                if result["status"] == "FAIL":
                    ...
                    self.log(f"    Expected: {result['expected']}, Got: {result['actual']}")
        
        self.log("\n" + "="*70)
        if self.failed_count == 0:
            self.log("ALL TESTS PASSED! [SUCCESS]", "PASS")
        else:
