...
def test_api_key_saved_to_qsettings():
    ...
    try:
        ...
        if loaded_key == test_key:
            ...
            logger.error(
                f"✗ API key mismatch: saved '{test_key}', loaded '{loaded_key}'"
            )
            return False
    ...
    try:
        ...
        if "gemini" in service.providers:
            ...
            if gemini_provider.api_key == "qsettings_key_12345":
                logger.info(
                    "✓ QSettings API key takes priority over environment variable"
                )
                return True
            else:
    ...
    try:
        ...
        if "gemini" in service.providers:
            ...
            if gemini_provider.api_key == "env_var_key_fallback":
                logger.info(
                    "✓ Falls back to environment variable when QSettings is empty"
                )
                return True
            else:
                logger.error(f"✗ Wrong API key: {gemini_provider.api_key}")
...
def main():
    ...
    logger.info("╔" + "=" * 68 + "╗")
    logger.info(
        "║" + " " * 12 + "API KEY LOADING FROM PREFERENCES TEST" + " " * 20 + "║"
    )
    logger.info("╚" + "=" * 68 + "╝")
    ...
    results.append(
        (
            "AI Service Loads API Key from QSettings",
            test_ai_service_loads_api_key_from_qsettings(),
        )
    )
    results.append(
        ("API Key Priority (QSettings > Environment)", test_api_key_priority())
    )
    results.append(
        ("API Key Fallback to Environment Variable", test_api_key_fallback_to_env())
    )
