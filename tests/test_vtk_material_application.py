...
class TestVTKMaterialApplication(unittest.TestCase):
    ...
    def test_pyqt3d_files_removed(self):
        ...
        for file_path in pyqt3d_files:
            ...
            self.assertFalse(full_path.exists(), f"PyQt3D file {file_path} should be removed")
    ...
    def test_model_renderer_material_methods(self):
        ...
        self.assertTrue(hasattr(renderer, 'apply_material'), "ModelRenderer should have apply_material method")
        self.assertTrue(hasattr(renderer, 'apply_default_material'), "ModelRenderer should have apply_default_material method")
        
        # Test apply_material method
        success = renderer.apply_material("test_material", self.mock_material_manager)
        self.assertTrue(success, "apply_material should return True when material manager succeeds")
        ...
        with patch('src.gui.viewer_3d.viewer_widget_facade.VTKSceneManager'):
            with patch('src.gui.viewer_3d.viewer_widget_facade.ModelRenderer'):
                with patch('src.gui.viewer_3d.viewer_widget_facade.CameraController'):
                    with patch('src.gui.viewer_3d.viewer_widget_facade.PerformanceTracker'):
                        with patch('src.gui.viewer_3d.viewer_widget_facade.ViewerUIManager'):
                            widget = Viewer3DWidget()
                            
                            # Test that material methods exist
                            self.assertTrue(hasattr(widget, 'apply_material_to_current_model'), 
                                          "Viewer3DWidget should have apply_material_to_current_model method")
                            self.assertTrue(hasattr(widget, 'apply_default_material_to_current_model'), 
                            ...
                            self.assertIsNone(material_manager, "_get_material_manager should return None for mock setup")
    
    def test_material_application_integration(self):
        """Test integration between viewer and material system."""
        with patch('src.gui.viewer_3d.viewer_widget_facade.VTKSceneManager'):
            with patch('src.gui.viewer_3d.viewer_widget_facade.ModelRenderer') as mock_model_renderer:
                with patch('src.gui.viewer_3d.viewer_widget_facade.CameraController'):
                    with patch('src.gui.viewer_3d.viewer_widget_facade.PerformanceTracker'):
                        with patch('src.gui.viewer_3d.viewer_widget_facade.ViewerUIManager'):
                            ...
                            mock_renderer_instance.apply_default_material.return_value = True
                            
                            widget = Viewer3DWidget()
                            
                            # Test material application
                            success = widget.apply_material_to_current_model("test_material", self.mock_material_manager)
                            self.assertTrue(success, "Material application should succeed")
                            
                            # Test default material application
                            success = widget.apply_default_material_to_current_model(self.mock_material_manager)
                            self.assertTrue(success, "Default material application should succeed")
    ...
    def test_material_preferences_integration(self):
        ...
        self.assertEqual(default_material, self.test_material, "Default material should be saved in preferences")
        
        # Test changing default material
        new_material = "cherry"
        self.settings.setValue('thumbnail/material', new_material)
        retrieved_material = self.settings.value('thumbnail/material', 'maple', type=str)
        self.assertEqual(retrieved_material, new_material, "Material preference should be changeable")
        ...
        for file_path in key_files:
            if os.path.exists(file_path):
                ...
                for pyqt3d_import in pyqt3d_imports:
                    self.assertNotIn(pyqt3d_import, content, 
    ...
    def test_material_lighting_integrator_method_names(self):
        ...
        try:
            ...
            self.assertFalse(hasattr(integrator, '_apply_stl_material_properties'), 
                           "MaterialLightingIntegrator should not have _apply_stl_material_properties method")
            self.assertFalse(hasattr(integrator, '_parse_mtl_direct'), 
