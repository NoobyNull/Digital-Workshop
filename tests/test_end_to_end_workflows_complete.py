...
"""
    ...
    def measure_workflow_performance(self, workflow_name: str, operation: callable) -> Dict[str, float]:
    ...
    def assert_workflow_performance(self, metrics: Dict[str, float], 
                                  max_time: float = 5.0, 
                                  max_memory_mb: float = 100.0):
        """Assert that workflow performance meets requirements."""
        self.assertLess(metrics["execution_time"], max_time,
                       f"Workflow took {metrics['execution_time']:.2f}s, expected < {max_time}s")
        self.assertLess(metrics["memory_used_mb"], max_memory_mb,
    ...
    def test_mixed_format_processing_workflow(self):
        ...
        def mixed_workflow():
            ...
            for model_info in processed_models:
                retrieved_model = self.model_repository.get_model(model_info["model_id"])
                self.assertIsNotNone(retrieved_model)
            
            return processed_models
        
        metrics = self.measure_workflow_performance("mixed_format_processing", mixed_workflow)
...
class TestErrorRecoveryWorkflows(EndToEndTestBase):
    ...
    def test_corrupted_file_recovery_workflow(self):
        ...
        metrics = self.measure_workflow_performance("corruption_recovery", corruption_recovery_workflow)
    ...
    def test_large_file_processing_workflow(self):
        ...
        metrics = self.measure_workflow_performance("large_file_processing", large_file_workflow)
...
class TestPerformanceValidationWorkflows(EndToEndTestBase):
    ...
    def test_sustained_load_workflow(self):
        ...
        metrics = self.measure_workflow_performance("sustained_load", sustained_load_workflow)
    ...
    def test_memory_efficiency_workflow(self):
        ...
        metrics = self.measure_workflow_performance("memory_efficiency", memory_efficiency_workflow)
        self.assert_workflow_performance(metrics, max_time=20.0, max_memory_mb=300.0)
