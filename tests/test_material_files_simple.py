...
def test_material_files_exist():
    ...
    for mtl_file in mtl_files:
        ...
        if png_file.exists():
            print(f"✅ {base_name}: MTL and PNG files found")
            ...
            try:
                with open(mtl_file, 'r') as f:
                    ...
                    if 'newmtl' in content:
                        print(f"  ✅ MTL file contains material definition")
                    else:
                        print(f"  ⚠️  MTL file may be incomplete (no 'newmtl' found)")
            ...
            print(f"❌ {base_name}: MTL file found but PNG file missing")
            success = False
...
def test_material_provider_basic():
    ...
    try:
        ...
        if len(materials) == 0:
            print("❌ No materials discovered by MaterialProvider")
            return False
        ...
        if retrieved:
            print(f"✅ Successfully retrieved material by name")
            return True
        else:
            print(f"❌ Failed to retrieve material by name")
            return False
            
    except Exception as e:
        print(f"❌ Error testing MaterialProvider: {e}")
        return False
...
def test_vtk_basic():
    ...
    try:
        ...
        print("✅ VTK actor created and configured successfully")
        return True
...
def main():
    ...
    if passed == total:
        print("\n🎉 All tests passed! Material application system is working correctly.")
        ...
        print(f"\n⚠️  {total - passed} test(s) failed. There may still be issues with material application.")
