...
class TestMaterialApplication(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.assertGreater(
            len(material_files), 0, "No material files found for testing"
        )
    ...
    def test_material_provider_discovery(self):
        ...
        for material in materials:
            ...
            self.assertTrue(
                material["texture_path"].exists(),
                f"Texture file not found: {material['texture_path']}",
            )

            # Check MTL file if it exists
            if material.get("mtl_path"):
                self.assertTrue(
                    material["mtl_path"].exists(),
                    f"MTL file not found: {material['mtl_path']}",
                )
    ...
    def test_complete_material_application_flow(self):
        ...
        if success:
            ...
            print(
                "⚠ Material application failed - this may be expected if texture files are missing"
            )
            ...
            try:
                ...
                if material and material.get("texture_path"):
                    if material["texture_path"].exists():
                        print(
                            "⚠ Texture file exists but application failed - possible VTK issue"
                        )
                    else:
