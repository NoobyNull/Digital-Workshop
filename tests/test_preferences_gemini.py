...
def test_provider_class_mapping():
    ...
    try:
        ...
        logger.info(
            f"✓ get_provider_class('anthropic') returned: {anthropic_class.__name__}"
        )
...
def test_ai_service_test_connection():
    ...
    try:
        ...
        success, message = AIDescriptionService.test_provider_connection(
            provider_id="gemini", api_key=api_key, model_id="gemini-2.5-flash"
        )
...
def main():
    ...
    logger.info("╔" + "=" * 68 + "╗")
    logger.info(
        "║" + " " * 12 + "PREFERENCES DIALOG GEMINI SUPPORT TEST" + " " * 20 + "║"
    )
    logger.info("╚" + "=" * 68 + "╝")
    ...
    if passed == total:
        logger.info(
            "✓ ALL TESTS PASSED - Preferences dialog Gemini support is working!"
        )
        return 0
    else:
