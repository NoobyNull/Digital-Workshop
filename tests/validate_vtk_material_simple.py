...
def validate_vtk_material_system():
    ...
    if pyqt3d_removed:
        success_count += 1
        print("  [OK] PyQt3D files successfully removed")
    else:
        print("  [FAIL] Some PyQt3D files still exist")
    ...
    for file_path in vtk_files:
        full_path = Path(file_path)
        if full_path.exists():
            print(f"  [OK] VTK file {file_path} exists")
            ...
            for pyqt3d_import in pyqt3d_imports:
                if pyqt3d_import in content:
                    print(f"    [FAIL] Found PyQt3D import: {pyqt3d_import}")
                    found_pyqt3d = True
                    vtk_files_exist = False

            if not found_pyqt3d:
                print(f"    [OK] No PyQt3D imports found")
        else:
            print(f"  [FAIL] VTK file {file_path} missing")
            vtk_files_exist = False

    if vtk_files_exist:
        success_count += 1
        print("  [OK] VTK viewer files are correct")
    else:
        print("  [FAIL] VTK viewer files have issues")
    ...
    if model_renderer_path.exists():
        ...
        required_methods = ["def apply_material(", "def apply_default_material("]

        methods_exist = True
        for method in required_methods:
            if method in content:
                print(f"  [OK] Found method: {method.strip()}")
            else:
                print(f"  [FAIL] Missing method: {method.strip()}")
                methods_exist = False

        if methods_exist:
            success_count += 1
            print("  [OK] ModelRenderer has material application methods")
        else:
            print("  [FAIL] ModelRenderer missing material application methods")
    else:
        print("  [FAIL] ModelRenderer file not found")
    ...
    if viewer_facade_path.exists():
        ...
        if methods_exist:
            success_count += 1
            print("  [OK] Viewer3DWidget has material application methods")
        else:
            print("  [FAIL] Viewer3DWidget missing material application methods")
    else:
        print("  [FAIL] Viewer3DWidget file not found")
    ...
    if integrator_path.exists():
        ...
        for method in correct_methods:
            if method in content:
                print(f"  [OK] Found correct method: {method.strip()}")
            else:
                print(f"  [FAIL] Missing correct method: {method.strip()}")
                methods_correct = False

        for method in incorrect_methods:
            if method in content:
                print(f"  [FAIL] Found incorrect method: {method.strip()}")
                methods_correct = False
            else:
                print(f"  [OK] Correctly absent: {method.strip()}")

        if methods_correct:
            success_count += 1
            print("  [OK] MaterialLightingIntegrator has correct method names")
        else:
            print("  [FAIL] MaterialLightingIntegrator has incorrect method names")
    else:
        print("  [FAIL] MaterialLightingIntegrator file not found")
    ...
    if materials_dir.exists():
        ...
        if mtl_files:
            print(f"  [OK] Found {len(mtl_files)} material files:")
            ...
            print("  [FAIL] No material files found")
    else:
        print("  [FAIL] Materials directory not found")
    ...
    if success_count == total_tests:
        print(
            "\n[SUCCESS] ALL TESTS PASSED! VTK-only material system is working correctly."
        )
        return True
    else:
        print(
            f"\n[WARNING] {total_tests - success_count} test(s) failed. Please review the issues above."
        )
        return False
