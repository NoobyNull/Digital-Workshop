...
def test_configuration_management(service):
    ...
    try:
        ...
        ), "Available providers should be a list"
...
def test_error_handling(service):
    ...
    try:
        ...
        try:
            ...
            logger.info(
                "✓ Correctly raised ValueError for unconfigured provider: %s",
                exception_message,
            )
        except Exception as e:
            exception_caught = True
            ...
            logger.info(
                "✓ Correctly raised %s for unconfigured provider: %s",
                exception_type,
                exception_message,
            )

        logger.info(
            "Exception caught: %s, message: %s", exception_caught, exception_message
        )

        assert (
            exception_caught
        ), f"Expected an exception for unconfigured provider, got: {exception_message}"
        assert (
            "provider" in exception_message
        ), f"Expected AI provider configuration error, got: {exception_message}"
