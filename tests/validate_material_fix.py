...
def validate_material_fix():
    ...
    integration_file = os.path.join(os.path.dirname(__file__), '..', 'src', 'gui', 'materials', 'integration.py')
    ...
    try:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef) and node.name == 'MaterialLightingIntegrator':
        ...
        for node in ast.walk(integrator_class):
            if isinstance(node, ast.FunctionDef) and node.name == 'apply_material_species':
                ...
                if isinstance(node.func, ast.Attribute):
                    if isinstance(node.func.value, ast.Name):
                        if node.func.value.id == 'self':
                            if node.func.attr in ['apply_stl_material_properties', 'parse_mtl_direct']:
                                method_calls_fixed.append(node.func.attr)
                                print(f"✅ Found correct method call: self.{node.func.attr}()")
        
        # Verify the fixes
        expected_methods = ['apply_stl_material_properties', 'parse_mtl_direct']
        expected_calls = ['apply_stl_material_properties']  # parse_mtl_direct is called from apply_stl_material_properties
        ...
        for call in expected_calls:
            if call not in method_calls_fixed:
                print(f"❌ ERROR: Method call self.{call}() not found in apply_material_species")
                success = False
            else:
        ...
        if success:
            ...
            print("✅ The critical bug preventing material application has been resolved")
        else:
