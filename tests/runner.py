...
class UnifiedTestRunner:
    ...
    def _discover_tests(self, test_paths: List[str]) -> List[str]:
        ...
        logger.info(
            f"Discovered {len(discovered_tests)} test files in paths: {test_paths}"
        )
    ...
    def _monitor_resources(
        self, process: subprocess.Popen, interval: float = 1.0
    ) -> Dict[str, float]:
    ...
    def run_test_suite(self, suite: TestSuiteConfig) -> TestExecutionResult:
        ...
        try:
            ...
            resource_monitor = threading.Thread(
                target=self._monitor_resources, args=(process,), daemon=True
            )
            ...
            if json_report_path.exists():
                try:
                    ...
                    if coverage_path.exists():
                        ...
                        root = tree.getroot()
                        coverage_percentage = (
                            float(root.attrib.get("line-rate", 0)) * 100
                        )
            ...
            logger.error(
                f"Test suite {suite.name} timed out after {suite.timeout} seconds"
            )
            if process:
                process.kill()
            return TestExecutionResult(
    ...
    def run_all_tests(
        ...
        """
        logger.info(
            f"Starting unified test execution with {len(self.test_suites)} suites"
        )
    ...
    def generate_unified_report(
        ...
        coverage_values = [
            r.coverage_percentage for r in results if r.coverage_percentage is not None
        ]
        avg_coverage = (
    ...
    def _generate_recommendations(
        ...
        if total_failed == 0:
            ...
            recommendations.append(
                f"Found {total_failed} failing tests that need attention."
            )
            ...
            if len(failed_suites) > len(results) * 0.5:
                recommendations.append(
                    "More than half of test suites have failures. Consider systematic fixes."
                )
            ...
            if timeout_suites:
                recommendations.append(
                    "Some tests are timing out. Consider optimizing slow tests or increasing timeouts."
                )
        ...
        if coverage_values:
            ...
            if avg_coverage < 80:
                recommendations.append(
                    f"Average coverage {avg_coverage:.1f}% is below 80%. Add more tests."
                )
            elif avg_coverage < 90:
                recommendations.append(
                    f"Average coverage {avg_coverage:.1f}% could be improved to 90%."
                )
        ...
        if total_duration > 1800:  # 30 minutes
            recommendations.append(
                "Total execution time exceeds 30 minutes. Consider parallel execution or test optimization."
            )
        ...
        return {
            ...
            "suite_performance": {
                result.suite_name: {
                    ...
                    "tests_per_second": (
                        result.total_tests / result.duration_seconds
    ...
    def print_summary(self, report: Dict[str, Any]):
        ...
        print(
            f"System: {summary['system_info']['cpu_count']} cores, {summary['system_info']['memory_total_gb']:.1f}GB RAM"
        )
        print(f"")
        print(f"Test Suites:")
        ...
        for gate in report["quality_gates"]:
            ...
            print(
                f"  {gate['name']}: {status} ({gate['actual']:.2f} vs {gate['threshold']})"
            )
        ...
        if report["failed_suites"]:
            ...
            for suite in report["failed_suites"]:
                print(
                    f"  - {suite['name']}: {suite['failed_tests']} failures ({suite['duration']:.2f}s)"
                )
...
def main():
    ...
    parser.add_argument(
        "--parallel-suites", action="store_true", help="Run test suites in parallel"
    )
    parser.add_argument(
        "--suite",
        type=str,
    ...
    try:
        ...
        results = runner.run_all_tests(
            parallel_suites=args.parallel_suites, max_workers=args.max_workers
        )
        ...
        if failed_gates:
            ...
            for gate in failed_gates:
                logger.warning(
                    f"  - {gate['name']}: {gate['actual']:.2f} (threshold: {gate['threshold']})"
                )

        # Return appropriate exit code
        overall_success = report["summary"]["failed_suites"] == 0 and all(
            gate["passed"] for gate in report["quality_gates"]
        )
