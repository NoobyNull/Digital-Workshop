        ...
        if 'def _apply_ai_results' in content:
            ...
            for old_field in old_fields:
                if old_field in method_content:
                    logger.error(f"✗ Old field name found in _apply_ai_results: {old_field}")
                    all_found = False
                else:
                    logger.info(f"✓ Old field name NOT in _apply_ai_results: {old_field}")
...
def test_error_handling():
    ...
    try:
        ...
        try:
            ...
            logger.info(f"✓ Correctly raised error for non-existent image: {type(e).__name__}")
