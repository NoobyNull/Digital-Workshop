# Test Plan: Code Formatting and Linting Validation

**Date**: 2025-10-31 | **Status**: Ready for Implementation | **Phase**: Phase 1

## Objective

Ensure code formatting and linting compliance of 95.56% or better through automated validation using Black (code formatting) and Pylint (linting analysis), with automated fixes and comprehensive reporting.

## Approach

### Technical Implementation Strategy

1. **Black Integration**: Use Black for consistent code formatting with automatic fixes
2. **Pylint Analysis**: Leverage Pylint for comprehensive linting and code quality analysis
3. **Compliance Scoring**: Implement weighted scoring system to achieve 95.56% target
4. **Automated Fixes**: Provide automatic formatting and fix suggestions
5. **Integration with CodeQuality Entity**: Map results to the data model for structured analysis

### Tools and Libraries

- **black**: Code formatter for consistent style
- **pylint**: Comprehensive linting and code analysis
- **isort**: Import sorting (optional enhancement)
- **mypy**: Static type checking (optional enhancement)
- **bandit**: Security linting (optional enhancement)
- **json**: For structured logging and reporting
- **pathlib**: For cross-platform file system operations

## Test Cases

### Test Case 1: Black Formatting Validation
**Objective**: Verify Black formatting compliance and automatic fixes

**Setup**:
```python
# Create test files with formatting issues
malformed_code.py           # Various formatting violations
properly_formatted.py       # Already compliant code
complex_formatting.py       # Complex formatting scenarios
```

**Expected Results**:
- Black detects and fixes formatting violations
- Formatted code passes Black validation
- No formatting regressions after fixes

### Test Case 2: Pylint Linting Analysis
**Objective**: Comprehensive linting analysis with scoring

**Setup**:
```python
# Create test files with various linting issues
high_quality_code.py        # Minimal linting issues
medium_quality_code.py      # Moderate linting issues
low_quality_code.py         # Many linting issues
```

**Expected Results**:
- Pylint provides detailed analysis and scoring
- Issues categorized by severity (error, warning, refactor, convention)
- Overall score meets 95.56% threshold

### Test Case 3: Combined Formatting and Linting
**Objective**: Validate integrated formatting and linting workflow

**Setup**:
- Run Black formatting first
- Then run Pylint analysis on formatted code
- Measure compliance improvement

**Expected Results**:
- Formatting fixes improve linting scores
- Combined workflow achieves target compliance
- No conflicts between tools

### Test Case 4: Large Codebase Performance
**Objective**: Ensure efficient processing of large codebases

**Setup**:
- Process codebase with 1000+ Python files
- Measure processing time and resource usage
- Test incremental processing (only changed files)

**Expected Results**:
- Full codebase processing < 60 seconds
- Incremental processing < 10 seconds
- Memory usage < 500MB

### Test Case 5: CI/CD Integration
**Objective**: Validate CI/CD pipeline integration

**Setup**:
- Configure GitHub Actions workflow
- Test on pull requests and pushes
- Verify quality gate enforcement

**Expected Results**:
- Pipeline fails when compliance < 95.56%
- Detailed reports generated
- Clear failure messages with actionable feedback

## Implementation Code

### Core Formatting and Linting Validator
        ...
        try:
            ...
            if result.stdout.strip():
                try:
                    ...
                    if result.stderr:
                        score_match = result.stderr.search(r"rated at ([0-9.]+)/10")
                        if score_match:
                            overall_score = float(score_match.group(1))
                            
                except json.JSONDecodeError:
                    logger.warning(f"Failed to parse Pylint JSON output for {file_path}")
...
"""
    ...
    def validate_directory(self, root_path: Path, fix_formatting: bool = True) -> List[Tuple[FormattingResult, LintingResult]]:
        """
    ...
    def calculate_compliance(self, results: List[Tuple[FormattingResult, LintingResult]]) -> ComplianceResult:
        ...
        if total_issues > len(results) * 5:  # More than 5 issues per file on average
            recommendations.append("High number of issues detected. Consider code review")
    ...
    def generate_report(self, results: List[Tuple[FormattingResult, LintingResult]], compliance: ComplianceResult) -> Dict[str, Any]:
        """
        ...
        total_processing_time = sum(f.processing_time + l.processing_time for f, l in results)
...
def main():
    ...
    parser.add_argument(
        '--no-fix', 
        action='store_true',
        help='Check only, do not apply fixes'
    )
    ...
    if target_path.is_file():
        ...
        formatting_result, linting_result = validator.validate_file(target_path, fix_formatting)
    ...
    print(f"Files with formatting issues: {report['summary']['files_with_formatting_issues']}")
    print(f"Files with linting errors: {report['summary']['files_with_linting_errors']}")
...
[MESSAGES CONTROL]
# Disable specific warnings that are not relevant or too strict
disable=
    C0114,  # missing-module-docstring
    C0115,  # missing-class-docstring
    C0116,  # missing-function-docstring
...
- [ ] Automatically format code using Black with configurable settings
- [ ] Comprehensive linting analysis using Pylint with detailed scoring
- [ ] Achieve 95.56% or better compliance score
- [ ] Provide automated fixes for formatting issues
- [ ] Generate detailed reports with actionable recommendations
- [ ] Support incremental validation (only changed files)

### Performance Requirements
- [ ] Process typical codebase (< 1000 files) in under 60 seconds
- [ ] Incremental processing < 10 seconds for small changes
- [ ] Memory usage stays below 500MB during validation
- [ ] Support parallel processing for large codebases

### Quality Requirements
- [ ] 100% accuracy in formatting detection and fixes
- [ ] Consistent linting scores across multiple runs
- [ ] Proper handling of edge cases (syntax errors, encoding issues)
- [ ] Cross-platform compatibility

### Integration Requirements
- [ ] Generate CodeQuality entities compatible with data model
...
jobs:
  code-quality:
    ...
    - name: Run Code Quality Validation
      run: |
        python tools/code_quality_validator.py src/ \
          --target-compliance 95.56 \
    ...
    - name: Upload Quality Report
      ...
      with:
        ...
        path: reports/code_quality_report.json
    
    - name: Check Compliance
      run: |
        python -c "
        import json
        with open('reports/code_quality_report.json') as f:
            report = json.load(f)
        
        compliance = report['summary']['compliance_percentage']
        if compliance < 95.56:
            print(f'Compliance rate {compliance:.2f}% is below 95.56% threshold!')
            exit(1)
...
class CodeQualityGate:
    """Quality gate for code formatting and linting."""
    
    def __init__(self, target_compliance: float = 95.56):
        self.target_compliance = target_compliance
        self.gate_id = "code-quality"
    
    def evaluate(self, compliance_result: ComplianceResult) -> QualityGateResult:
        """Evaluate if the code passes the quality gate."""
        return QualityGateResult(
            gate_id=self.gate_id,
            passed=compliance_result.passed,
            violations=compliance_result.violations,
            compliance_rate=compliance_result.actual_compliance
        )
```
...
```python
def generate_fix_script(results: List[Tuple[FormattingResult, LintingResult]]) -> str:
    """Generate a script to automatically fix formatting issues."""
    script_lines = ["#!/bin/bash", "echo 'Applying code quality fixes...'"]
    
    # Add Black formatting commands
    for formatting_result, _ in results:
        if formatting_result.was_formatted and not formatting_result.error_message:
            script_lines.append(f"# Format {formatting_result.file_path}")
            script_lines.append(f"black {formatting_result.file_path}")
    
    script_lines.append("echo 'Code quality fixes applied.'")
    return "\n".join(script_lines)
```
...
repos:
  ...
  - repo: https://github.com/pycqa/pylint
    ...
    hooks:
      - id: pylint
        args: [--rcfile=.pylintrc, --score=yes]
  
  - repo: local
    hooks:
      - id: code-quality-check
        name: Code Quality Check
        entry: python tools/code_quality_validator.py
        language: system
        files: \.py$
        args: [--target-compliance, 95.56]
```
