...
def main():
    """Generate report for src directory only."""
    src_dir = Path('src')
    py_files = sorted(src_dir.rglob('*.py'))
    
    print(f"Linting {len(py_files)} Python files in src/...\n")
    ...
    print("\n" + "="*80)
    print("LINTING REPORT - SRC DIRECTORY ONLY")
    print("="*80)
    ...
    if below_95:
        print("\n" + "="*80)
        print("FILES BELOW 9.5 SCORE (sorted by score)")
        print("="*80)
        below_95.sort(key=lambda x: x[1])
        for filepath, score in below_95[:50]:
            print(f"{score:6.2f}/10 - {filepath}")
        
        if len(below_95) > 50:
            print(f"\n... and {len(below_95) - 50} more files")
