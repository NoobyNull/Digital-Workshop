...
class CodeMigrator:
    ...
    def __init__(self, root_path: str = "src", dry_run: bool = False, create_backups: bool = True):
    ...
    def validate_migration(self) -> Dict[str, Any]:
        ...
        for file_path in files:
            try:
                ...
                validation['hardcoded_styles_remaining'].extend(issues['hardcoded_styles'])
            except Exception as e:
                self.logger.error(f"Failed to validate {file_path}: {e}")

        # Calculate completeness
        total_issues = len(validation['legacy_imports_remaining']) + len(validation['hardcoded_styles_remaining'])
        if total_issues == 0:
    ...
    def _get_python_files(self) -> List[str]:
        ...
        for py_file in self.root_path.rglob('*.py'):
            ...
            if any(part in ['__pycache__', '.git', 'venv', 'env', '.venv', '.env', 'tests']
    ...
    def _apply_import_pattern(self, content: str, lines: List[str], pattern: str,
                            replacement: str, migration_type: str, file_path: str) -> Tuple[str, List[ImportMigration]]:
    ...
    def _migrate_styles_in_file(self, file_path: str) -> List[StyleMigration]:
        ...
        stylesheet_pattern = re.compile(r'(\.setStyleSheet\(["\'])([^"\']+)(["\'])', re.MULTILINE | re.DOTALL)
    ...
    def _migrate_stylesheet_content(self, stylesheet: str, file_path: str,
                                   line_num: int, full_content: str) -> Tuple[str, Optional[StyleMigration]]:
    ...
    def _validate_file_migration(self, file_path: str) -> Dict[str, List[str]]:
        ...
        stylesheet_calls = re.findall(r'\.setStyleSheet\(["\']([^"\']+)["\']', content, re.MULTILINE | re.DOTALL)
        for call in stylesheet_calls:
            ...
            if hardcoded_colors and 'COLORS.' not in call and '{' not in call:
                issues['hardcoded_styles'].append(f"{file_path}: {len(hardcoded_colors)} hardcoded colors")
    ...
    def generate_report(self, import_migrations: List[ImportMigration] = None,
                       style_migrations: List[StyleMigration] = None) -> MigrationReport:
        """
        ...
        if summary['legacy_imports_remaining'] > 0:
            warnings.append(f"{summary['legacy_imports_remaining']} legacy imports still need migration")

        if summary['hardcoded_styles_remaining'] > 0:
            warnings.append(f"{summary['hardcoded_styles_remaining']} hardcoded styles still need migration")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Migration Utilities for Theme Refactoring')
    parser.add_argument('command', choices=['update-imports', 'migrate-styles', 'validate-migration', 'generate-report'],
    ...
    parser.add_argument('--output', '-o', default='migration_report.json',
                       help='Output file for reports')
    parser.add_argument('--root-path', default='src',
                       help='Root path to process (default: src)')
    ...
    if args.command == 'update-imports':
        ...
        print(f"Legacy imports remaining: {len(validation['legacy_imports_remaining'])}")
        print(f"Hardcoded styles remaining: {len(validation['hardcoded_styles_remaining'])}")
        ...
        print(f"Migration completeness: {report.summary['migration_completeness']:.1f}%")
        print(f"Legacy imports remaining: {report.summary['legacy_imports_remaining']}")
        print(f"Hardcoded styles remaining: {report.summary['hardcoded_styles_remaining']}")
