...
class ReinstallMode:
    ...
    def _backup_user_data(self) -> Path:
        ...
        if self.installer.data_dir.exists():
            ...
            logger.info("Data backed up: %s", data_backup)

        # Backup config directory
        if self.installer.config_dir.exists():
            config_backup = backup_dir / "config"
            shutil.copytree(self.installer.config_dir, config_backup)
            logger.info("Config backed up: %s", config_backup)

        logger.info("User data backup created: %s", backup_dir)
        return backup_dir
    ...
    def _remove_app_files(self):
        ...
        if self.installer.modules_dir.exists():
            ...
            logger.debug("Removed modules directory: %s", self.installer.modules_dir)

        # Remove logs directory
        if self.installer.logs_dir.exists():
            shutil.rmtree(self.installer.logs_dir)
            logger.debug("Removed logs directory: %s", self.installer.logs_dir)

        # Remove manifest and version files
        if self.installer.manifest_file.exists():
            self.installer.manifest_file.unlink()
            logger.debug("Removed manifest file: %s", self.installer.manifest_file)

        if self.installer.version_file.exists():
            self.installer.version_file.unlink()
            logger.debug("Removed version file: %s", self.installer.version_file)
    ...
    def _restore_user_data(self, backup_dir: Path):
        ...
        if data_backup.exists():
            ...
            logger.info("Data restored: %s", self.installer.data_dir)
        ...
        if config_backup.exists():
            ...
            logger.info("Config restored: %s", self.installer.config_dir)
    ...
    def _verify_installation(self):
        ...
        if not self.installer.data_dir.exists():
            logger.warning("Data directory missing: %s", self.installer.data_dir)
