    ...
    def create_backup(self, backup_name: Optional[str] = None) -> Optional[Path]:
        ...
        try:
            ...
            if self.installer.manifest_file.exists():
                ...
                logger.debug(f"Manifest backed up: {manifest_backup}")
            ...
            if self.installer.version_file.exists():
                ...
                logger.debug(f"Version file backed up: {version_backup}")
            
            # Create backup metadata
            metadata = {
                "backup_name": backup_name,
                "created": datetime.now().isoformat(),
                "version": self.installer.version_file.read_text().strip() if self.installer.version_file.exists() else "unknown",
            }
    ...
    def restore_backup(self, backup_dir: Path) -> bool:
        ...
        try:
            ...
            if not self.verify_backup(backup_dir):
                logger.error(f"Backup verification failed: {backup_dir}")
                return False
            ...
            if manifest_backup.exists():
                ...
                logger.debug(f"Manifest restored: {self.installer.manifest_file}")
            
            # Restore version file
            version_backup = backup_dir / "version.txt"
            if version_backup.exists():
                shutil.copy2(version_backup, self.installer.version_file)
                logger.debug(f"Version file restored: {self.installer.version_file}")
            
            logger.info(f"Backup restored: {backup_dir}")
            return True
        ...
        try:
            ...
            for required_file in required_files:
                ...
                if not file_path.exists():
                    logger.warning(f"Required file missing in backup: {required_file}")
            ...
            if metadata_file.exists():
                ...
                logger.debug(f"Backup metadata: {metadata}")
            
            logger.info(f"Backup verified: {backup_dir}")
            return True
