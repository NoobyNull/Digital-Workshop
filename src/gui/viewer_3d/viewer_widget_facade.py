...
class Viewer3DWidget(QWidget):
    ...
    def _set_render_mode(self, mode: RenderMode) -> None:
        ...
        try:
            ...
            if self.render_window:
                ...
                if not success:
                    self.logger.warning(
                        f"Render failed for mode {mode.value}, continuing anyway"
                    )
            else:
    ...
    def load_model(self, model: Model, progress_callback=None) -> bool:
        ...
        try:
            self.logger.info(
                f"Loading model with {model.stats.triangle_count} triangles"
            )
    ...
    def _calculate_z_up_rotation_from_model_bounds(self) -> tuple:
        ...
        try:
            ...
            self.logger.info(
                f"Model dimensions - X: {dx:.2f}, Y: {dy:.2f}, Z: {dz:.2f}"
            )
    ...
    def _get_material_manager(self):
        ...
        try:
            ...
            try:
                ...
                self.logger.debug(
                    f"Could not create fallback material manager: {fallback_error}"
                )
    ...
    def _on_material_selected(self, material_name: str) -> None:
        ...
        try:
            ...
            if success:
                ...
                self.logger.warning(
                    f"Failed to apply selected material: {material_name}"
                )
