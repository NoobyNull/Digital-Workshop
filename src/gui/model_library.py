...
class ModelLibraryWidget(QWidget):
    ...
    def _create_file_browser(self, parent_layout: QVBoxLayout) -> None:
        ...
        self.add_root_folder_button = QPushButton("Add Root Folder")
        self.add_root_folder_button.setToolTip("Add a new root folder to browse")
        self.add_root_folder_button.clicked.connect(self._add_root_folder_from_context)
        import_layout.addWidget(self.add_root_folder_button)
    ...
    def _load_models(self, file_paths: List[str]) -> None:
        ...
        op_id = self.performance_monitor.start_operation(
    ...
    def _on_load_finished(self) -> None:
        ...
        if self._disposed:
            if self.model_loader:
                try:
                    try:
                        self.model_loader.model_loaded.disconnect(self._on_model_loaded)
                    except Exception:
                        pass
                    try:
                        ...
                        pass
                    try:
                        self.model_loader.error_occurred.disconnect(self._on_load_error)
                    except Exception:
                        pass
                    try:
                        self.model_loader.finished.disconnect(self._on_load_finished)
                    except Exception:
                        pass
        ...
        try:
            ...
            if len(selected_models) == 1:
                ...
                regenerate_action.triggered.connect(
                    lambda: self._regenerate_thumbnail(int(model_id))
                )
                ...
                show_file_action.triggered.connect(
                    lambda: self._show_file_in_explorer(int(model_id))
                )
        ...
        try:
            ...
            if index.isValid():
                ...
                if is_root_folder:
                    ...
                    remove_root_action.triggered.connect(
                        lambda: self._remove_root_folder_from_context(node.root_folder)
                    )
        ...
        try:
            ...
            if reply == QMessageBox.Yes:
                if self.root_folder_manager.remove_folder(root_folder.id):
                    QMessageBox.information(
                        self, "Success", f"Removed folder '{root_folder.display_name}'"
                    )
    ...
    def _update_model_metadata(self, model_id: int) -> None:
        ...
        try:
            ...
            save_close_btn.clicked.connect(
                lambda: self._save_metadata_and_close(dialog, editor, model_id)
            )
        ...
        try:
            ...
            if field == "Category":
                ...
                value, ok = QInputDialog.getText(
                    self, f"Set {field}", f"Enter {field.lower()} for {count} models:"
                )
    ...
    def _import_selected_files(self) -> None:
        ...
        try:
            ...
            if not selected_indexes:
                QMessageBox.information(
                    self, "Import", "Please select one or more files to import."
                )
                return
    ...
    def _import_selected_folder(self) -> None:
        ...
        try:
            ...
            if reply == QMessageBox.Yes:
                # Recursive search - check both lowercase and uppercase extensions
                for ext in supported_extensions:
                    files_to_import.extend(folder_path.rglob(f"*{ext}"))
                    files_to_import.extend(folder_path.rglob(f"*{ext.upper()}"))
            else:
                # Non-recursive search (only immediate folder) - check both cases
                for ext in supported_extensions:
                    files_to_import.extend(folder_path.glob(f"*{ext}"))
                    files_to_import.extend(folder_path.glob(f"*{ext.upper()}"))
            ...
            if not files_to_import:
                QMessageBox.information(
                    self,
                    "Import",
                    f"No supported model files found in the selected folder.\nSupported formats: {', '.join(supported_extensions)}",
                )
                return
        ...
        try:
            ...
            if not has_triangles and has_vertex_array:
                QMessageBox.warning(
                    self,
                    "Warning",
                    "This model uses array-based geometry and cannot be analyzed for errors at this time.",
                )
                return
    ...
    def _regenerate_thumbnail(self, model_id: int) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Regenerating thumbnail with preferences: material={material}, bg_image={bg_image}, bg_color={bg_color}"
            )
