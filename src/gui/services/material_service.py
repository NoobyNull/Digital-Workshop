...
class MaterialValidationWorker(QThread):
    ...
    def run(self) -> None:
        ...
        try:
            ...
            if result.is_valid:
                ...
                self.logger.warning(f"Material validation failed: {self.material_name} - {result.error_message}")
    ...
    def _validate_material_data(self) -> MaterialValidationResult:
        ...
        try:
            ...
            if not name:
                return MaterialValidationResult(False, "Material name cannot be empty", warnings, suggestions)
            ...
            if not isinstance(properties, dict):
                return MaterialValidationResult(False, "Properties must be a dictionary", warnings, suggestions)
            
            # Check property types
            numeric_properties = ['density', 'hardness', 'thermal_conductivity', 'electrical_conductivity']
            ...
            if not preview_path:
                suggestions.append("Consider adding a preview image for better material identification")
...
class MaterialService(IMaterialService):
    ...
    def create_material_from_template(self, template_name: str, 
                                    custom_params: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    def _on_validation_completed(self, material_name: str, result: MaterialValidationResult) -> None:
        ...
        try:
            ...
            if not result.is_valid:
                self.logger.warning(f"Material validation failed: {material_name} - {result.error_message}")
            if result.warnings:
                self.logger.info(f"Material validation warnings: {material_name} - {result.warnings}")
