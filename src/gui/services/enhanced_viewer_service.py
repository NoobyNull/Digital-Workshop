...
class ModelLoadingWorker(QThread):
    ...
    def __init__(self, file_path: Path, viewer_widget: Viewer3DWidget, 
    ...
    def run(self) -> None:
        ...
        try:
            ...
            for step in range(estimated_steps):
                ...
                progress = ProgressInfo(step, estimated_steps, f"Loading model... ({step}/{estimated_steps})")
            ...
            if model:
                ...
                if success:
                    model_info = {
                        "filename": model.filename if hasattr(model, "filename") else self.file_path.name,
                        "file_size": file_size,
                        "triangle_count": getattr(model, 'stats', {}).triangle_count if hasattr(model, 'stats') else 0
                    }
    ...
    def _create_dummy_model(self) -> Optional[Model]:
        ...
        try:
            ...
            class DummyModel:
                def __init__(self):
                    self.filename = self.file_path.name if hasattr(self, 'file_path') else "model.stl"
...
class EnhancedViewerService(IEnhancedViewerService):
    ...
    def _setup_performance_thresholds(self) -> None:
        ...
        self.logger.info(f"Performance mode: {self.performance_mode}, target FPS: {self.target_fps}")
    
    def load_model_async(self, file_path: Path, 
                        progress_callback: Optional[Callable[[ProgressInfo], None]] = None,
                        cancellation_token: Optional[Callable[[], bool]] = None) -> bool:
        ...
        try:
            ...
            if file_size > 500 * 1024 * 1024:  # 500MB
                self.ui_service.show_warning("Large File", 
            ...
            self.current_loading_worker = ModelLoadingWorker(file_path, self.viewer_widget, cancellation_check)
    ...
    def _get_user_friendly_error_message(self, error_type: str, message: str) -> str:
        ...
        return error_messages.get(error_type, f"An error occurred while loading the file: {message}")
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        try:
            stats = {
                ...
                "loading_worker_active": self.current_loading_worker is not None and self.current_loading_worker.isRunning(),
