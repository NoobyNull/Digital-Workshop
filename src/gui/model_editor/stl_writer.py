    ...
    def write_ascii(model: STLModel, output_path: str) -> bool:
        ...
        try:
            ...
            with open(output_path, "w") as f:
                ...
                for triangle in model.triangles:
                    f.write(
                        f"  facet normal {triangle.normal.x:.6e} {triangle.normal.y:.6e} {triangle.normal.z:.6e}\n"
                    )
                    f.write("    outer loop\n")
                    f.write(
                        f"      vertex {triangle.v1.x:.6e} {triangle.v1.y:.6e} {triangle.v1.z:.6e}\n"
                    )
                    f.write(
                        f"      vertex {triangle.v2.x:.6e} {triangle.v2.y:.6e} {triangle.v2.z:.6e}\n"
                    )
                    f.write(
                        f"      vertex {triangle.v3.x:.6e} {triangle.v3.y:.6e} {triangle.v3.z:.6e}\n"
                    )
                    f.write("    endloop\n")
                    f.write("  endfacet\n")
