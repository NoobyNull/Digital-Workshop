...
class QtMaterialThemeCore(QObject):
    ...
    def get_qt_material_colors(self, theme: str, variant: str) -> Dict[str, str]:
        ...
        try:
            ...
            logger.warning(
                f"Theme/variant not found: {theme}/{variant}, using dark/blue"
            )
    ...
    def get_all_colors(self, theme: str = None, variant: str = None) -> Dict[str, str]:
        ...
        for app_color_name, qt_color_name in self._color_mappings.items():
            app_colors[app_color_name] = qt_material_colors.get(
                qt_color_name, "#1976D2"
            )

        return app_colors

    def get_qcolor(
        self, color_name: str, theme: str = None, variant: str = None
    ) -> QColor:
        """
    ...
    def set_theme(self, theme: str, variant: str = None) -> None:
        ...
        logger.info(
            f"Theme changed from {old_theme}/{old_variant} to {theme}/{variant}"
        )
    ...
    def save_settings(self) -> None:
        ...
        logger.debug(
            f"Theme settings saved: {self._current_theme}/{self._current_variant}"
        )

    def load_settings(self) -> None:
        """Load theme settings."""
        self._current_theme = self.settings.value(
            "theme/current_theme", "dark", type=str
        )
        self._current_variant = self.settings.value(
            "theme/current_variant", "blue", type=str
        )

        # Validate loaded settings
        if self._current_theme not in self._qt_material_themes:
            self._current_theme = "dark"
        if self._current_variant not in self._qt_material_themes.get(
            self._current_theme, {}
        ):
            self._current_variant = "blue"

        logger.debug(
            f"Theme settings loaded: {self._current_theme}/{self._current_variant}"
        )
    ...
    def log_theme_info(self) -> None:
        ...
        logger.debug(
            f"Available variants for {theme}: {list(self._qt_material_themes[theme].keys())}"
        )
