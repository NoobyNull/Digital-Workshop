...
class EnhancedVTKContextManager:
    ...
    def detect_context_loss_early(self, render_window: vtk.vtkRenderWindow) -> Tuple[bool, ContextState]:
        """
        ...
        try:
            ...
            if cache_key in self.context_cache:
                ...
                if current_time - last_check < self.detection_interval:
                    ...
                    return cached_state in [ContextState.LOST, ContextState.DESTROYING], cached_state
    ...
    def _validate_context_internal(self, render_window: vtk.vtkRenderWindow, operation: str) -> ContextState:
    ...
    def set_shutdown_scenario(self, scenario: ShutdownScenario) -> None:
        ...
        with self._lock:
            ...
            self.logger.info(
                f"Shutdown scenario changed: {old_scenario.value} -> {scenario.value}"
            )
    ...
    def coordinate_cleanup_sequence(self, render_window: vtk.vtkRenderWindow) -> bool:
        ...
        try:
            ...
            for i, callback in enumerate(self.cleanup_callbacks):
                try:
                    self.logger.debug(f"Executing cleanup callback {i+1}/{len(self.cleanup_callbacks)}")
                    result = callback()
                    if not result:
    ...
    def _verify_cleanup_completion(self) -> None:
        ...
        try:
            with self._lock:
                ...
                self.logger.info(
                    f"Cleanup verification: VTK={vtk_status}, OpenGL={opengl_status}"
                )
...
def detect_context_loss_early(render_window: vtk.vtkRenderWindow) -> Tuple[bool, ContextState]:
    """
...
def coordinate_shutdown_cleanup(render_window: vtk.vtkRenderWindow, 
                               scenario: ShutdownScenario = ShutdownScenario.NORMAL_SHUTDOWN) -> bool:
    """
