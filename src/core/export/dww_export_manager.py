...
class DWWExportManager:
    ...
    def _add_metadata_to_archive(self, dww_archive: zipfile.ZipFile, project_id: str, files: List[Dict]) -> None:
    ...
    def _create_manifest(self, project: Dict, files: List[Dict]) -> Dict:
        ...
        return {
            ...
            "files": [
                {
                    ...
                    "size": Path(f.get('file_path')).stat().st_size if Path(f.get('file_path')).exists() else 0,
            ...
            "file_count": len([f for f in files if f.get('file_path') and Path(f.get('file_path')).exists()]),
        }
        ...
        try:
            with zipfile.ZipFile(dww_path, 'r') as dww_archive:
                ...
                if calculated_hash == stored_hash:
                    ...
                    return False, "DWW file integrity check failed - file may be corrupted"
