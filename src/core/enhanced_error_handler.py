...
class EnhancedErrorHandler(IErrorHandler):
    ...
    def should_retry(self, error: Exception) -> bool:
        ...
        try:
            ...
            return (error_info.should_retry and 
                    error_info.retry_count < error_info.max_retries and
                    error_info.is_transient)
    ...
    def _categorize_error(self, error: Exception) -> ErrorCategory:
        ...
        # Database errors
        if any(keyword in error_message for keyword in ["database", "sql", "sqlite", "connection", "timeout"]):
        ...
        # GPU errors
        if any(keyword in error_message for keyword in ["gpu", "opengl", "vulkan", "cuda", "graphics"]):
        ...
        # Security errors
        if any(keyword in error_message for keyword in ["security", "permission", "unauthorized", "forbidden"]):
            return ErrorCategory.SECURITY_ERROR
        
        # Performance errors
        if any(keyword in error_message for keyword in ["performance", "slow", "timeout", "deadlock"]):
    ...
    def _analyze_retry_properties(self, error: Exception, category: ErrorCategory) -> tuple[bool, bool]:
        """Analyze if error should be retried and if it's transient."""
        # Transient errors that can be retried
        transient_errors = [
            "ConnectionError", "TimeoutError", "TemporaryFileNotFoundError",
            "DatabaseError", "NetworkError"
        ]
        
        error_type = type(error).__name__
        is_transient = error_type in transient_errors or category in [
            ErrorCategory.NETWORK_ERROR, ErrorCategory.DATABASE_ERROR
        ]
    ...
    def _log_error(self, error_info: ErrorInfo) -> None:
        ...
        if (self.enable_security_logging and 
            error_info.category == ErrorCategory.SECURITY_ERROR):
