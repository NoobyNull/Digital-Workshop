        ...
        try:
            ...
            for resource in list(self._tracked_resources):
                try:
                    ...
                    self.logger.warning(f"Failed to cleanup tracked resource {type(resource).__name__}: {e}")
    ...
    def register_network_connection(self, connection) -> None:
        ...
        try:
            if connection and hasattr(connection, 'close'):
                ...
                self.logger.debug(f"Registered network connection for cleanup: {type(connection).__name__}")
            else:
