    ...
    def handle_cleanup_error(self, error: Exception, context: str) -> bool:
        ...
        try:
            ...
            if self._error_counts[error_key] == 1:
                ...
                self.logger.warning(f"Repeated cleanup error in {context} ({self._error_counts[error_key]} times): {error}")
            else:
                self.logger.error(f"Frequent cleanup error in {context} ({self._error_counts[error_key]} times): {error}")
