...
class ComprehensiveLogger:
    ...
    def _create_log_entry(self, level: str, message: str, 
                         ...
                         exception_info: Optional[Any] = None) -> Dict[str, Any]:
    ...
    def _log_structured(self, logger: logging.Logger, level: str, message: str,
                       context: Optional[LogContext] = None,
                       extra_data: Optional[Dict[str, Any]] = None,
    ...
    def log_warning(self, message: str, context: Optional[LogContext] = None,
                   extra_data: Optional[Dict[str, Any]] = None):
        """Log warning message."""
        with self._lock:
    ...
    def log_error(self, message: str, context: Optional[LogContext] = None,
        ...
        """Log error message."""
        with self._lock:
            self._log_structured(self.error_logger, "ERROR", message, context, extra_data, exception_info)
    
    def log_critical(self, message: str, context: Optional[LogContext] = None,
                    extra_data: Optional[Dict[str, Any]] = None,
                    exception_info: Optional[Any] = None):
        """Log critical message."""
        with self._lock:
            self._log_structured(self.error_logger, "CRITICAL", message, context, extra_data, exception_info)
    
    def log_debug(self, message: str, context: Optional[LogContext] = None,
                 extra_data: Optional[Dict[str, Any]] = None):
        """Log debug message."""
        with self._lock:
            self._log_structured(self.app_logger, "DEBUG", message, context, extra_data)
    
    def log_security(self, message: str, context: Optional[LogContext] = None,
                    extra_data: Optional[Dict[str, Any]] = None):
    ...
    def end_operation(self, operation_id: str, success: bool = True,
        ...
        with self._lock:
            ...
            if success:
                self.log_info(f"Operation completed: {operation_id}", log_context, performance_data)
            else:
    ...
    def log_error_report(self, error_report, context: Optional[LogContext] = None):
        ...
        error_data = {
            ...
            "recovery_strategy": error_report.classification.recovery_strategy.value if error_report.classification.recovery_strategy else None
        }
        
        if error_report.classification.severity == ErrorSeverity.CRITICAL:
            self.log_critical(f"Critical error reported: {error_report.error_type}", 
                            log_context, error_data)
        else:
            self.log_error(f"Error reported: {error_report.error_type}", 
                         log_context, error_data)
    ...
    def operation_context(self, operation_name: str, 
        ...
        try:
            ...
            self.end_operation(operation_id, success=False, context=context, 
...
def log_info(message: str, context: Optional[LogContext] = None,
            extra_data: Optional[Dict[str, Any]] = None):
    """Log info message using global logger."""
    get_global_logger().log_info(message, context, extra_data)


def log_warning(message: str, context: Optional[LogContext] = None,
               extra_data: Optional[Dict[str, Any]] = None):
    """Log warning message using global logger."""
    get_global_logger().log_warning(message, context, extra_data)


def log_error(message: str, context: Optional[LogContext] = None,
             extra_data: Optional[Dict[str, Any]] = None,
             exception_info: Optional[Any] = None):
...
def log_critical(message: str, context: Optional[LogContext] = None,
                extra_data: Optional[Dict[str, Any]] = None,
                exception_info: Optional[Any] = None):
...
def log_debug(message: str, context: Optional[LogContext] = None,
             extra_data: Optional[Dict[str, Any]] = None):
