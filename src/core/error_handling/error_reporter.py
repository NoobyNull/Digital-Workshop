...
from .error_categories import (
    ErrorReport, ErrorSeverity, ErrorCategory,
    ErrorRecoveryStrategy, ErrorContext
)
...
class ErrorReporter:
    ...
    def _setup_default_handlers(self) -> None:
        ...
        self.recovery_strategies[ErrorRecoveryStrategy.IMMEDIATE_SHUTDOWN] = self._immediate_shutdown
        self.recovery_strategies[ErrorRecoveryStrategy.RETRY_WITH_BACKOFF] = self._retry_with_backoff
        self.recovery_strategies[ErrorRecoveryStrategy.FALLBACK_MODE] = self._fallback_mode
        self.recovery_strategies[ErrorRecoveryStrategy.USER_INTERVENTION] = self._request_user_intervention
        self.recovery_strategies[ErrorRecoveryStrategy.IGNORE_AND_CONTINUE] = self._ignore_and_continue
    ...
    def report_error(self, error: Exception, context: ErrorContext,
                    ...
                    recovery_callback: Optional[Callable] = None) -> ErrorReport:
        """
        ...
        try:
            ...
            self.logger.critical(
                f"Error reporting failed: {reporting_error}",
                exc_info=True
            )
            raise
    ...
    def _update_error_patterns(self, error_report: ErrorReport) -> None:
        ...
        self.error_patterns[error_key] = [
            ts for ts in self.error_patterns[error_key]
    ...
    def _update_reporting_stats(self, reporting_time: float) -> None:
        ...
        with self._lock:
            ...
            self.reporting_stats["average_report_time"] = (
                (current_avg * (total_reports - 1) + reporting_time) / total_reports
            )
    ...
    def _handle_system_resource_error(self, error_report: ErrorReport) -> None:
        ...
        self.logger.error(
            f"System resource error: {error_report.error_message}"
        )
    ...
    def _request_user_intervention(self, error_report: ErrorReport, recovery_callback: Callable) -> bool:
...
def report_error(error: Exception, context: ErrorContext,
                context_info: Dict[str, Any] = None,
                system_info: Dict[str, Any] = None,
                user_context: Dict[str, Any] = None,
                recovery_callback: Optional[Callable] = None) -> ErrorReport:
