        ...
        try:
            if self.service:
                ...
                if result.status == AnalysisStatus.COMPLETED:
                    ...
                    self.analysis_failed.emit(self.model_id, result.error_message or "Unknown error")
...
class ImportAnalysisService:
    ...
    def start_analysis(
        ...
        Args:
            ...
            cancellation_token: Optional cancellation token
        """
        self._log_json("analysis_started", {
            "model_id": model_id,
    ...
    def start_batch_analysis(
        ...
        Args:
            ...
            cancellation_token: Optional cancellation token
        """
        self._log_json("batch_analysis_started", {
    ...
    def _analyze_single_model(
        ...
        try:
            ...
            result = self._perform_geometry_analysis(
                model, model_id, file_path, start_time,
            ...
            self._log_json("analysis_failed", {
                "model_id": model_id,
                "file": file_name,
                "error": str(e),
    ...
    def _perform_geometry_analysis(
        ...
        unique_vertices, vertex_count_actual = self._count_unique_vertices(model.triangles if model.triangles else [])
        ...
        degenerate_count = self._count_degenerate_triangles(model.triangles if model.triangles else [])
        ...
        volume = self._calculate_volume(model.triangles if model.triangles else [], non_manifold_count == 0)
        ...
        self._log_json("analysis_completed", {
            "model_id": model_id,
    ...
    def _create_cancelled_result(
        ...
        self._log_json("analysis_cancelled", {
            "model_id": model_id,
            "file": Path(file_path).name,
            "analysis_time_seconds": round(analysis_time, 3)
        })
    ...
    def _analyze_batch_internal(
        ...
        self._log_json("batch_analysis_completed", {
