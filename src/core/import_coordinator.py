...
from src.core.import_file_manager import (
    ...
    ImportSession, ImportResult, ImportFileInfo
)
from src.core.import_thumbnail_service import (
    ImportThumbnailService, StorageLocation,
    ThumbnailGenerationResult, ThumbnailBatchResult
)
from src.core.import_analysis_service import (
    ImportAnalysisService, GeometryAnalysisResult,
    BatchAnalysisResult, AnalysisStatus
)
...
class ImportCoordinatorWorker(QThread):
    ...
    def _generate_thumbnails(
        self,
        processed_files: List[ImportFileInfo]
    ) -> Optional[ThumbnailBatchResult]:
        """Generate thumbnails for processed files."""
        self._update_stage(
            ImportWorkflowStage.THUMBNAIL_GENERATION,
    ...
    def _queue_analysis(
        ...
        self._update_stage(
            ImportWorkflowStage.BACKGROUND_ANALYSIS,
            "Queueing background analysis..."
        )
        
        try:
            # Prepare file-model pairs for analysis
            file_model_pairs = [
                (model['file_path'], idx + 1)
                for idx, model in enumerate(stored_models)
            ]
