...
class FloatCacheAnalyzer:
            ...
            return {
                ...
                "throughput": {
                    ...
                    "triangles_per_second": (
                        ...
                        if sum(m.total_parse_time for m in self.metrics) > 0
                        else 0
                    ),
                },
            }
    ...
    def validate_performance_assumptions(self) -> Dict[str, bool]:
        ...
        if "timing" in summary:
            validations["float_decode_is_bottleneck"] = (
                summary["timing"]["float_decode_percentage"] > 20
            )
        ...
        if "cache_performance" in summary:
            validations["cache_provides_significant_savings"] = (
                summary["cache_performance"]["time_savings_percentage"] > 30
            )

        # Assumption 4: Cache hit ratio is reasonable (>50% for repeated files)
        if "summary" in summary:
            validations["cache_hit_ratio_is_reasonable"] = (
                summary["summary"]["cache_hit_ratio"] > 0.5
            )

        # Assumption 5: Memory overhead is acceptable (<2x file size)
        if "memory" in summary and "summary" in summary:
            avg_file_mb = (
                summary["summary"]["total_file_size_mb"]
        ...
        self.logger.info(
            ...
            extra={"validations": validations, "summary": summary},
        )
