...
class DatabaseHealthMonitor:
    ...
    def _collect_system_metrics(self) -> None:
        ...
        try:
            ...
            self._record_metric('system.cpu.usage', cpu_percent, MetricType.GAUGE, '%',
                              threshold_warning=80, threshold_critical=95)
            
            # Memory usage
            memory = psutil.virtual_memory()
            self._record_metric('system.memory.usage', memory.percent, MetricType.GAUGE, '%',
                              threshold_warning=80, threshold_critical=95)
            self._record_metric('system.memory.available', memory.available / (1024**3), MetricType.GAUGE, 'GB')
            
            # Disk usage
            disk_usage = psutil.disk_usage(os.path.dirname(self.db_path))
            disk_percent = disk_usage.used / disk_usage.total
            self._record_metric('system.disk.usage', disk_percent * 100, MetricType.GAUGE, '%',
                              threshold_warning=80, threshold_critical=95)
            self._record_metric('system.disk.free', disk_usage.free / (1024**3), MetricType.GAUGE, 'GB')
    ...
    def _collect_database_metrics(self) -> None:
        ...
        try:
            with self._create_connection() as conn:
                ...
                self._record_metric('database.integrity', integrity_status, MetricType.GAUGE, 'boolean')
                ...
                self._record_metric('database.metadata.count', metadata_count, MetricType.GAUGE, 'count')
                
                # Connection statistics
                cursor.execute("PRAGMA database_list")
                databases = cursor.fetchall()
                self._record_metric('database.connections', len(databases), MetricType.GAUGE, 'count')
                
                # WAL mode status
                cursor.execute("PRAGMA journal_mode")
                journal_mode = cursor.fetchone()[0]
                self._record_metric('database.journal_mode', 1.0 if journal_mode.upper() == 'WAL' else 0.0,
                                  MetricType.GAUGE, 'boolean')
                
        except Exception as e:
            logger.error(f"Failed to collect database metrics: {str(e)}")

    def _record_metric(self, name: str, value: float, metric_type: MetricType, unit: str,
                      threshold_warning: Optional[float] = None,
                      threshold_critical: Optional[float] = None,
    ...
    def record_query_execution(self, query: str, execution_time: float, 
        ...
        with self._lock:
            ...
            if query_hash in self._query_performance:
                ...
                if cache_hit:
                    perf.cache_hit_rate = (perf.cache_hit_rate * (perf.execution_count - 1) + 1) / perf.execution_count
                else:
                    perf.cache_hit_rate = (perf.cache_hit_rate * (perf.execution_count - 1)) / perf.execution_count
            else:
            ...
            if execution_time > self._thresholds['query_time_critical']:
                logger.critical(f"Critical slow query detected: {execution_time:.2f}s - {query[:100]}")
    ...
    def get_current_health_status(self) -> HealthStatus:
        ...
        with self._lock:
            ...
            critical_alerts = [a for a in self._alerts.values() if a.status == HealthStatus.CRITICAL and not a.resolved]
            if critical_alerts:
                return HealthStatus.CRITICAL
            
            # Check for warning alerts
            warning_alerts = [a for a in self._alerts.values() if a.status == HealthStatus.WARNING and not a.resolved]
            if warning_alerts:
                return HealthStatus.WARNING
            
            return HealthStatus.HEALTHY

    def get_health_metrics(self, metric_name: Optional[str] = None, 
                          time_range: Optional[timedelta] = None) -> List[HealthMetric]:
        """
        Get health metrics.
    ...
    def generate_health_report(self) -> DatabaseHealthReport:
        ...
        with self._lock:
            ...
            system_resources = {
                'cpu_percent': self._current_metrics.get('system.cpu.usage', HealthMetric('', 0, MetricType.GAUGE, '', datetime.now())).value,
                'memory_percent': self._current_metrics.get('system.memory.usage', HealthMetric('', 0, MetricType.GAUGE, '', datetime.now())).value,
                'disk_percent': self._current_metrics.get('system.disk.usage', HealthMetric('', 0, MetricType.GAUGE, '', datetime.now())).value,
            }
            
            # Generate recommendations
            recommendations = self._generate_recommendations(current_metrics, active_alerts, query_performance)
    ...
    def _generate_recommendations(self, metrics: Dict[str, HealthMetric], 
                                alerts: List[HealthAlert],
                                query_performance: List[QueryPerformance]) -> List[str]:
        """
        ...
        if slow_queries:
            recommendations.append(f"Consider optimizing {len(slow_queries)} slow queries (avg time > 1s)")
        
        # Check for high error rates
        error_prone = [q for q in query_performance if q.execution_count > 0 and q.error_count / q.execution_count > 0.05]
        if error_prone:
        ...
        if cpu_metric and cpu_metric.value > 80:
            recommendations.append("High CPU usage detected - consider optimizing queries or upgrading hardware")
        ...
        if disk_metric and disk_metric.value > 80:
            recommendations.append("High disk usage detected - consider cleaning up old data or expanding storage")
        
        # Check database integrity
        integrity_metric = metrics.get('database.integrity')
        if integrity_metric and integrity_metric.value < 1.0:
            recommendations.append("Database integrity issues detected - run integrity check and repair if necessary")
        
        # Check connection count
        conn_metric = metrics.get('database.connections')
        if conn_metric and conn_metric.value > 20:
            recommendations.append("High number of database connections - consider connection pooling")
