    ...
    def update_metadata(self, model_id: str, metadata: Dict[str, Any]) -> bool:
        ...
        try:
            with self._get_connection() as conn:
                ...
                regular_fields = {'title', 'description', 'keywords', 'category', 'source', 'rating'}
        ...
        try:
            with self._get_connection() as conn:
                ...
                if key.startswith('camera_'):
                    ...
                    cursor.execute(f"SELECT camera_{camera_key} FROM model_metadata WHERE model_id = ?", (model_id,))
                else:
                    cursor.execute(f"SELECT {key} FROM model_metadata WHERE model_id = ?", (model_id,))
        ...
        try:
            with self._get_connection() as conn:
                ...
                if success:
                    ...
                    logger.warning(f"No metadata found to increment view count for model {model_id}")
