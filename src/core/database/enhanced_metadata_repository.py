...
class EnhancedMetadataRepository(IMetadataRepository):
    ...
    def get_metadata_value(self, model_id: str, key: str) -> Optional[Any]:
        ...
        try:
            ...
            logger.error(
                "Failed to get metadata value %s for model {model_id}: {str(e)}", key
            )
            return None
    ...
    def set_metadata_value(self, model_id: str, key: str, value: Any) -> bool:
        ...
        try:
            with self._get_connection() as conn:
                ...
                if success:
                    logger.debug(
                        "Set metadata value %s = {value} for model {model_id}", key
                    )
                else:
                    logger.warning(
                        "No metadata record found to update for model %s", model_id
                    )
            ...
            logger.error(
                "Failed to set metadata value %s for model {model_id}: {str(e)}", key
            )
            return False
