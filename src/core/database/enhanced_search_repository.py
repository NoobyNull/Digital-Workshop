...
class EnhancedSearchRepository(ISearchRepository):
    ...
    def search_by_file_type(self, file_types: List[str]) -> List[str]:
        ...
        try:
            with self._get_connection() as conn:
                ...
                model_ids = [str(row[0]) for row in rows]
                logger.debug(f"File type search returned {len(model_ids)} results for types: {file_types}")
                return model_ids
    ...
    def get_search_suggestions(self, partial_query: str) -> List[str]:
        ...
        try:
            ...
            logger.debug(f"Generated {len(sorted_suggestions)} search suggestions for: '{partial_query}'")
