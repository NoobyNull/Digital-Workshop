...
class QueryOptimizer:
    ...
    def analyze_query(self, query: str, params: tuple = None) -> QueryPlan:
        ...
        try:
            with self._get_connection() as conn:
                ...
                logger.debug(f"Query analyzed: {execution_time:.3f}s, cost: {cost:.2f}, rows: {rows_examined}")
                return query_plan
    ...
    @log_function_call(logger)
    def execute_optimal_query(self, query: str, params: tuple = None, use_cache: bool = True) -> List[tuple]:
        """
    ...
    def analyze_and_optimize(self) -> Dict[str, Any]:
        ...
        try:
            ...
            slow_queries = [
                pattern for pattern, stats in self._query_stats.items()
    ...
    def get_slow_queries(self, threshold: float = 0.1) -> List[Tuple[str, QueryStats]]:
        ...
        with self._lock:
            return [
                (pattern, stats) for pattern, stats in self._query_stats.items()
...
class IndexManager:
        ...
        try:
            with self._get_connection() as conn:
                ...
                indexes_to_create = [
                    ...
                    ("idx_metadata_category_rating", "model_metadata", "category, rating"),
                    ("idx_metadata_category_view_count", "model_metadata", "category, view_count"),
        ...
        try:
            with self._get_connection() as conn:
                ...
                if analysis['total_indexes'] == 0:
                    analysis['recommendations'].append("No indexes found - consider creating performance indexes")
    ...
    def get_unused_indexes(self, min_usage: int = 10) -> List[str]:
        ...
        with self._lock:
            ...
            used_indexes = {
                name for name, count in self._index_usage.items()
                if count >= min_usage
            }
