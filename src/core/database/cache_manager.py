...
class DiskCache:
        ...
        try:
            with open(cache_path, 'rb') as f:
                ...
                if metadata.get('ttl_seconds'):
                    age = (datetime.now() - datetime.fromisoformat(metadata['created_at'])).total_seconds()
    ...
    def put(self, key: str, value: Any, ttl_seconds: Optional[float] = None,
            tags: List[str] = None, compress: bool = True) -> bool:
        """
...
class DatabaseCacheManager:
    ...
    def _log_cache_stats(self) -> None:
        ...
        logger.info(f"Cache stats - Memory: {memory_stats.entries_count} entries, "
                   f"{memory_stats.memory_usage_mb:.1f}MB, Hit rate: {memory_stats.hit_rate:.2%}")
        logger.info(f"Cache stats - Disk: {disk_stats.entries_count} entries, "
    ...
    def cache_search_results(self, query: str, results: List[str], filters: Dict[str, Any] = None) -> None:
        """
    ...
    def optimize_cache_performance(self) -> Dict[str, Any]:
        ...
        if disk_stats.entries_count == 0:
            results['recommendations'].append("Disk cache appears empty - check cache configuration")
