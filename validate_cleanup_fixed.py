#!/usr/bin/env python3
"""
Digital Workshop Root Directory Cleanup Validation Script (Fixed)
...
Usage: python validate_cleanup_fixed.py
"""
...
class CleanupValidator:
    ...
    def check_root_file_count(self):
        ...
        try:
            ...
            expected_max = 30  # Allow some flexibility for essential files
    ...
    def check_no_broken_references(self):
        ...
        try:
            if os.path.exists('src'):
                # Use Python to search for patterns instead of grep
                            ...
                            try:
                                with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                                    ...
                                    if test_pattern.search(content):
                                        broken_refs.append(f"Found potential hardcoded test file references in {file_path}")
                                        break
                            except Exception:
                                continue
                                
        except Exception as e:
            self.log_warning(f"Could not check for test references: {e}")
            
        # Check for references to old report locations
        try:
            if os.path.exists('src'):
                report_pattern = re.compile(r'reports/')
                for root, dirs, files in os.walk('src'):
                    for file in files:
                        if file.endswith('.py'):
                            file_path = os.path.join(root, file)
                            try:
                                with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                                    ...
                                    if report_pattern.search(content):
                                        broken_refs.append(f"Found potential hardcoded report references in {file_path}")
                                        break
                            except Exception:
                                continue
                                
        except Exception as e:
            self.log_warning(f"Could not check for report references: {e}")
